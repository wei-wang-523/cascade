...
public class ExpressionManagerImpl extends AbstractExpressionManager {
	 ...
	 * CVC4 in a counter-example.
	 */
	private final ConcurrentMap<String, InductiveTypeImpl> inductiveTypeCache = new MapMaker()
			.makeMap();
	...
	/**
	 * NOTE: CVC will not create arrays with boolean elements. TODO: Wrap
	 * ('a,boolean) arrays as ('a -> boolean) functions?
	 */
	@Override
	...
	@Override
	public FunctionType functionType(Iterable<? extends Type> args, Type range) {
	...
	@Override
	public TupleExpression tuple(Type type, Expression first,
			Expression... rest) {
	...
	@Override
	public RecordTypeImpl recordType(String tname, String elementName,
			Type elementType) {
	...
	@Override
	public RecordExpression record(Type r1, Expression first,
			Expression... rest) {
		Preconditions.checkArgument(rest.length > 0);
	...
	private BooleanExpressionImpl toBooleanExpression(Expr e)
	...
	public FunctionExpression lambda(Expression arg, Expression body) {
		return FunctionExpressionImpl.create(this, Collections.singletonList(arg),
				body);
	}
	...
	ExpressionImpl toExpression(Expr e) throws TheoremProverException {
		if (e.getKind() == edu.nyu.acsys.CVC4.Kind.PLUS) {
			...
			return importExpression(extract(toExpression(e.getChild(1)), low, high));
			...
			return importExpression(tuple(toType(e.getType()), args));
		} else if (e.getKind() == edu.nyu.acsys.CVC4.Kind.RECORD) {
			Vector<Expression> args = new Vector<Expression>();
			for (int i = 0; i < e.getNumChildren(); i++)
				args.add(toExpression(e.getChild(i)));
			return importExpression(record(toType(e.getType()), args));
	...
	TypeImpl toType(edu.nyu.acsys.CVC4.Type type) {
		if (type.isBoolean()) {
			...
			edu.nyu.acsys.CVC4.ArrayType arrayType = new edu.nyu.acsys.CVC4.ArrayType(
					type);
			...
			edu.nyu.acsys.CVC4.FunctionType funcType = new edu.nyu.acsys.CVC4.FunctionType(
					type);
			...
			edu.nyu.acsys.CVC4.BitVectorType bvType = new edu.nyu.acsys.CVC4.BitVectorType(
					type);
			int size = (int) bvType.getSize();
			return bitVectorType(size);
		} else if (type.isRecord()) {
			edu.nyu.acsys.CVC4.RecordType recordType = new edu.nyu.acsys.CVC4.RecordType(
					type);
			...
			for (int i = 0; i < size; i++) {
				Object[] pair = record.getField(i);
				fieldNames.add((String) pair[0]);
				fieldTypes.add(toType((edu.nyu.acsys.CVC4.Type) pair[1]));
			}
