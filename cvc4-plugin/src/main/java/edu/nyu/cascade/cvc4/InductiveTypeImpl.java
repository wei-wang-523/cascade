...
public class InductiveTypeImpl extends TypeImpl implements InductiveType {
	...
	private static class Builder {
		...
		public ImmutableList<InductiveTypeImpl> build(
					...
					for (String constructorName : constructorNameLists.get(i)) {
						...
						if (selectorSize > 0) {
							...
							for (String selectorName : selectorNameListLists.get(i).get(j)) {
								...
								edu.nyu.acsys.CVC4.Type sType = selectorTypeListLists.get(i)
										.get(j).get(k);
								edu.nyu.acsys.CVC4.DatatypeUnresolvedType usType = selectorUnresolvedTypeListLists
										.get(i).get(j).get(k);
								try {
									if (sType != null) {
			...
			try {
				...
				assert typeNames.size() == selectorUnresolvedTypeListLists
						.size() : "expected names and unresolved types vectors to be of equal length";
				...
				for (int i = 0; i < typeNames.size(); i++) {
					...
					assert constructorNameLists.get(i)
							.size() == selectorUnresolvedTypeListLists.get(i)
									.size() : "expected sub-vectors in constructors and unresolved types vectors to match in size";

					for (int j = 0; j < constructorNameLists.get(i).size(); j++) {
						DatatypeConstructor cons = new DatatypeConstructor(
								constructorNameLists.get(i).get(j));
			...
			for (int i = 0; i < cvcTypes.size(); i++) {
				...
				InductiveTypeImpl t = new InductiveTypeImpl(exprManager, cvcType,
	...
	static class ConstructorImpl implements Constructor {
		static ConstructorImpl valueOf(Constructor constructor) {
			if (constructor instanceof ConstructorImpl) {
				...
				// return new
				// Constructor(constructor.getName(),constructor.getSelectors());
			}
		}
	...
	static class SelectorImpl implements Selector {
		...
		static SelectorImpl valueOf(Selector selector) {
			if (selector instanceof SelectorImpl) {
				...
				throw new UnsupportedOperationException();
				// return new
				// Selector<T>(selector.getName(),selector.getTypeDescriptor());
			}
		}
	...
	static InductiveTypeImpl createInductiveType(
			ExpressionManagerImpl expressionManager, String name,
			Constructor... constructors) {
		@SuppressWarnings("unchecked")
		List<InductiveTypeImpl> types = recursiveTypes(expressionManager,
				ImmutableList.of(name), Lists.newArrayList(constructors));
		assert (types.size() == 1);
		return types.get(0);
	}
	 ...
	 * String name, List<String> constructors, List<List<String>> selectorLists,
	 * List<List<IExpression>>> selectorTypeLists) throws Exception { if(
	 * constructors.size() != selectorLists.size() || selectorLists.size() !=
	 * selectorTypeLists.size() ) { throw new IllegalArgumentException(
	 * "Inconsistent arguments"); } ExprManager em =
	 * expressionManager.getTheoremProver().getValidityChecker(); TypeMut type =
	 * em.dataType(name,constructors,selectorLists,selectorTypeLists); return new
	 * InductiveType(expressionManager,type,name,constructors,selectorLists,
	 * selectorTypeLists); }
	 */
	/*
	 * public static InductiveType create(ExpressionManager expressionManager,
	 * String name, String constructor, List<String> selectors, List<IExpression>>
	 * selectorTypes) throws Exception { return create(expressionManager, name,
	 * Collections.singletonList(constructor),
	 * Collections.singletonList(selectors),
	 * Collections.singletonList(selectorTypes)); }
	 */
	/*
	 * public static Map<String,InductiveType> recursiveTypes(ExpressionManager
	 * expressionManager, List<String> names, List<List<String>> constructorLists,
	 * List<List<List<String>>> selectorListLists, List<List<List<IExpression>>>>
	 * selectorTypeListLists) throws Exception { if (names.size() !=
	 * constructorLists.size() || constructorLists.size() !=
	 * selectorListLists.size() || selectorListLists.size() !=
	 * selectorTypeListLists.size()) { throw new IllegalArgumentException(
	 * "Inconsistent arguments"); }
	 * 
	 * ExprManager em = expressionManager.getTheoremProver().getValidityChecker();
	 * 
	 * @SuppressWarnings("unchecked") List<TypeMut> cvc_types = em.dataType(names,
	 * constructorLists, selectorListLists, selectorTypeListLists); assert(
	 * cvc_types.size() == names.size() );
	 * 
	 * 
	 * Iterator<TypeMut> typeIter = cvc_types.iterator(); Iterator<String>
	 * nameIter = names.iterator(); Iterator<List<String>> constrIter =
	 * constructors.iterator(); Iterator<List<List<String>>> selIter =
	 * selectors.iterator();
	 * 
	 * int i=0; Map<String,InductiveType> typeMap =
	 * Maps.newHashMapWithExpectedSize(names.size()); for( TypeMut type :
	 * cvc_types ) { InductiveType t = new InductiveType(expressionManager, type,
	 * names.get(i), constructorLists.get(i), selectorListLists.get(i),
	 * selectorTypeListLists.get(i)); typeMap.put(names.get(i), t); i++; }
	 * 
	...
	private InductiveTypeImpl(ExpressionManagerImpl expressionManager,
			edu.nyu.acsys.CVC4.Type cvcType, String name,
	...
	/*
	 * private InductiveType(ExpressionManager expressionManager, TypeMut type,
	 * String name, List<String> constructors, List<List<String>> selectorLists,
	 * List<List<IExpression>>> selectorTypeLists) throws Exception {
	 * super(expressionManager); setCVC4Type(type);
	 * 
	 * this.name = name; this.constructors = Lists.newArrayList(constructors);
	 * this.selectors = Maps.newHashMap(); this.selectorTypes = Maps.newHashMap();
	 * 
	 * Map constructors -> selectors int i=0; for( String constructor :
	 * constructors ) { List<String> selectorList = selectorLists.get(i);
	 * selectors.put(constructor, selectorList);
	 * 
	 * List<IExpression>> selectorTypeList = selectorTypeLists.get(i); if(
	 * selectorList.size() != selectorTypeList.size() ) { throw new
	 * IllegalArgumentException("Inconsistent arguments"); }
	 * 
	 * Map selectors -> types int j=0; for( String selector : selectorList ) {
	 * selectorTypes.put(selector, selectorTypeList.get(j)); j++; } i++; } }
	 */
	...
	/*
	 * @Override public boolean isStub() { return stub; }
	 */
	/*
	 * @Override public List<String> getSelectors(String constr) { return
	 * selectors.get(constr); }
	 */
	...
	@Override
	public Expression select(Selector s, Expression x) {
	...
	public InductiveBoundVariableImpl boundVar(final String name, boolean fresh) {
		return new InductiveBoundVariableImpl(getExpressionManager(), name, this,
				fresh);
	}

	@Override
	public InductiveBoundVariableImpl boundExpression(final String name,
