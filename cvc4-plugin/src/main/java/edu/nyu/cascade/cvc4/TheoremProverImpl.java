...
public class TheoremProverImpl implements TheoremProver {
	...
	public SatResult<?> checkSat(Expression expr) {
		...
		try {
			...
			if (IOUtils.debugEnabled()) {
				...
				IOUtils.debug().pln("Simplified: " + cvc4ExprSimp).flush();
			}
	...
	ExprManager getCvc4ExprManager() {
		if (cvc4ExprManager == null) {
			/**
			 * FIXME: default loading library "cvc4" with linked error to find
			 * edu.nyu.acsys.CVC4.CVC4JNI.new_ExprManager__SWIG_0()
			 */
	...
	private void initializePreferences() {
		...
		smtEngine.setOption("interactive-mode", new SExpr("true"));
