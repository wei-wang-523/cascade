...
public final class FunctionVariableImpl
  extends VariableExpressionImpl
...
*/  
  ...
  private FunctionVariableImpl(ExpressionManagerImpl exprManager, Kind kind,
      Expr expr, String name, FunctionType type) {
    super(exprManager, kind, expr, name, type);
  }
  
  private FunctionVariableImpl(ExpressionManagerImpl exprManager, String name, final FunctionType ftype,boolean fresh) {
    super(exprManager, new VariableConstructionStrategy() {
      ...
      public Expr apply(ExprManager em, String name, edu.nyu.acsys.CVC4.Type type)
          throws Exception {
        /** FIXME: for some reason, the unit test TheoryIdTest would fail with following
         * commented lines: type.toString() cause runtime termination  
         */
        TheoremProverImpl.debugCommand("%s: %s;", name, type);
        TheoremProverImpl.tpFileCommand("%s: %s;", name, type);
