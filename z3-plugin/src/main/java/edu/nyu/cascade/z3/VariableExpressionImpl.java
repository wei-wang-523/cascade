...
class VariableExpressionImpl extends ExpressionImpl
	...
	static VariableExpressionImpl valueOf(ExpressionManagerImpl exprManager,
			Expression e) {
		if (e instanceof VariableExpressionImpl) {
			...
			return new VariableExpressionImpl(exprManager, name, ei.getType(), false);
		} else {
	...
	private VariableExpressionImpl(ExpressionManagerImpl exprManager, Expr expr,
			String name, Type type) {
		super(exprManager, VARIABLE, expr, type);
		setName(name);
	}

	protected VariableExpressionImpl(final ExpressionManagerImpl exprManager,
		...
		super(exprManager, new VariableConstructionStrategy() {
			...
			public Expr apply(Context ctx, String name, Sort sort)
					throws Z3Exception {
				/*
				 * TODO: see if var is already defined. There's a bug in lookupVar bc
				 * it's second parameter is a output parameter. Need to change the API
				 * so that it only takes the name.
				 */
				try {
	...
	protected VariableExpressionImpl(ExpressionManagerImpl exprManager,
			VariableConstructionStrategy strategy, String name, Type type,
			boolean fresh) {
