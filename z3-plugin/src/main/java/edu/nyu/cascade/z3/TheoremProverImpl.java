...
public class TheoremProverImpl implements TheoremProver {
	...
	@Override
		...
		.of(Option.builder().longOpt(OPTION_PBQI) //
		...
		.build(), //
		    Option.builder().longOpt(OPTION_Z3_STATS)
		    .desc("Show z3 statistics.")
		    .build());
	}
    ...
    static void z3FileCommand(String prefix, Expr z3Expr, Sort z3Type, String postfix) {
	if(IOUtils.tpFileEnabled()) {
	    PrintStream stream = IOUtils.tpFileStream().append(prefix);
	    stream.append(z3Expr.toString());
	    stream.append(" ");
	...
	try {
		...
		IOUtils.debug().pln(
				    "Simplified: " + z3Expr.simplify()).flush();
	    }
