...
public class ControlFile {
	...
	public static ControlFile fromXml(File file) throws ControlFileException {
		try {
			edu.nyu.cascade.control.jaxb.ControlFile jaxbControlFile = (edu.nyu.cascade.control.jaxb.ControlFile) UNMARSHALLER
					.unmarshal(file);
			ControlFile.Builder cfBuilder = new ControlFile.Builder();
			Map<BigInteger, File> sourceFiles = Maps.newHashMap();
			for (edu.nyu.cascade.control.jaxb.ControlFile.SourceFile sourceFile : jaxbControlFile
					.getSourceFile()) {
			...
			edu.nyu.cascade.control.jaxb.ControlFile.Theory theory = jaxbControlFile
					.getTheory();
			if (theory != null) {
				cfBuilder.setTheoryId(TheoryId.valueOf(theory));
			}

			for (edu.nyu.cascade.control.jaxb.ControlFile.Run run : jaxbControlFile
					.getRun()) {
	...
	private ControlFile(ImmutableList<File> sourceFiles, /*
																												 * ImmutableList<
																												 * TheoryId> theories
																												 */TheoryId theoryId,
			ImmutableList<Run> runs) {
	...
	public static class Builder {
		...
		public ControlFile build() {
			return new ControlFile(sourceFiles.build(), theoryId /*
																														 * theories.build()
																														 */, runs.build());
		}
	}
	...
	/*
	 * public ImmutableList<TheoryId> getTheories() { return theories; }
	 */

	public TheoryId getTheoryId() {
		return theoryId;
	}
}
