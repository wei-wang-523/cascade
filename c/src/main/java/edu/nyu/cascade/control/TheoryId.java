...
public class TheoryId {
	...
	public edu.nyu.cascade.c.theory.Theory getInstance(
			ExpressionManager exprManager) throws ControlFileException {
		try {
			Class<?> theoryClass = Class.forName(qname);
			Constructor<?> theoryConstr = theoryClass.getConstructor(
					ExpressionManager.class);
			return (edu.nyu.cascade.c.theory.Theory) theoryConstr.newInstance(
					exprManager);
