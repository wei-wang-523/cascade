...
public class Position extends AbstractLocation implements IRLocation {
	 ...
	 * edu.nyu.cascade.control.jaxb.Position position, Map<BigInteger, File>
	 * sourceFiles) throws ControlFileException { return
	 * valueOf(position,sourceFiles,InsertionType.BEFORE); }
	 * 
	 */
	static edu.nyu.cascade.control.Position valueOf(
		...
		return new Position(position.getCommand(), file, position.getLine(),
				position.getInsert(), functions, loops);
	}
	...
	public Position(List<Command> commands, File file, BigInteger line,
			InsertionType insertionType, List<CallPoint> functions,
			List<LoopPoint> loops) {
