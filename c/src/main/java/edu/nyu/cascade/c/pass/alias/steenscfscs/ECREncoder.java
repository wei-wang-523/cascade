...
public class ECREncoder extends Visitor {
	 ...
	 */
	private final Map<Triple<String, String, String>, ECR> ecrMap = Maps
			.newHashMap();
	private final Map<Triple<GNode, String, String>, ECR> opECRMap = Maps
			.newHashMap();
	...
	private class LvalVisitor extends Visitor {
		...
		public ECR visitEnumerator(GNode node) {
			...
			Triple<String, String, String> key = Triple.of(id, scopeName,
		...
		public ECR visitPrimaryIdentifier(GNode node) throws PreProcessorException {
			...
			while (scopeItr.hasNext()) {
				...
				Triple<String, String, String> parentKey = Triple.of(id, scopeName,
						parentScope);
				if (ecrMap.containsKey(parentKey))
					return ecrMap.get(parentKey);
			}

			// The return assign statement (lhs := rhs), while rhs belongs to the last
			// scope
			String lastScope = CScopeAnalyzer.getLastScope();
			Triple<String, String, String> lastKey = Triple.of(id, scopeName,
					lastScope);
			if (ecrMap.containsKey(lastKey))
				return ecrMap.get(lastKey);
	...
	private class RvalVisitor extends Visitor {
			...
			if (Preferences.isSet(Preferences.OPTION_CFS_POINTER_ARITH)) {
				...
				if (type.resolve().isPointer()) {
					...
					if (lhsType.resolve().isPointer() && rhsType.hasConstant()) {
						...
						Triple<GNode, String, String> key = Triple.of(node, CType
								.getScopeName(node), CScopeAnalyzer.getCurrentScope());
						opECRMap.put(key, resECR);

						long targetSize = cTypeAnalyzer.getSize(type.resolve().toPointer()
								.getType());
						long srcSize = cTypeAnalyzer.getSize(lhsType.resolve().toPointer()
								.getType());
		...
		public ECR visitCastExpression(GNode node) {
			Triple<GNode, String, String> key = Triple.of(node, CType.getScopeName(
					node), CScopeAnalyzer.getCurrentScope());
	...
	private ECR getComponent(ECR srcECR, Type baseType, String fieldName) {
		Preconditions.checkArgument(baseType.hasTag(Tag.STRUCT) || baseType.hasTag(
				Tag.UNION));
	...
	private ECR getOpECR(GNode node, ECR leftECR, ECR rightECR) {
		...
		while (scopeItr.hasNext()) {
			String parentScope = scopeItr.next();
			Triple<GNode, String, String> parentKey = Triple.of(node, scopeName,
					parentScope);
			if (opECRMap.containsKey(parentKey))
				return opECRMap.get(parentKey);
		}

		// The return assign statement (lhs := rhs), while rhs belongs to the last
		// scope
		String lastScope = CScopeAnalyzer.getLastScope();
		Triple<GNode, String, String> lastKey = Triple.of(node, scopeName,
				lastScope);
		if (opECRMap.containsKey(lastKey))
			return opECRMap.get(lastKey);
	...
	private ECR pointerCast(ECR srcECR, Type srcType, Type targetType) {
		...
		long targetPtr2Size = targetPtr2Type.isVoid() ? 0
				: cTypeAnalyzer.getSize(targetPtr2Type);
		...
		if (uf.containsPtrAritJoin(srcECR, srcPtr2Size)) {
			/*
			 * Replace the one created for pointer arithmetic operations. Sample code:
			 * (typeof(*msg) *)((char *)__mptr - (size_t)&((typeof(*msg) *)0)->list)
			 */
