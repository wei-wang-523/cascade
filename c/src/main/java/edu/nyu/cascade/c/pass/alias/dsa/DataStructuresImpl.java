...
abstract class DataStructuresImpl extends DataStructures {
	...
	DSGraph getOrCreateGraph(Function F) {
		...
		if (Clone) {
			...
			// Note that this graph is the graph for ALL of the function in the SCC,
			// not
			// just F.
			for (Entry<Function, DSNodeHandle> entry : G.getReturnNodes()
					.entrySet()) {
				if (entry.getKey() != F)
					DSInfo.put(entry.getKey(), G);
			}
		}
	...
	void buildGlobalECs(Set<GlobalValue> ECGlobals) {
		...
		for (DSNode N : GlobalsGraph.getNodes()) {
			...
			if (GlobalECs.findValue(First) != null) {
				...
				if (!FirstLeader.equals(First))
					GlobalECs.unionSets(FirstLeader, First);
				First = FirstLeader;
			}
	...
	@Override
	DataStructures init(DataStructures D, boolean clone, boolean useAuxCalls,
			boolean copyGlobalAuxCalls, boolean resetAux) {
	...
	void cloneIntoGlobals(DSGraph G, int cloneFlags) {
		...
		ReachabilityCloner RC = new ReachabilityCloner(GlobalsGraph, G, cloneFlags,
				true);
	...
	void formGlobalFunctionList() {
		...
		GlobalFunctionList.addAll(List);
		List.clear();
	}
