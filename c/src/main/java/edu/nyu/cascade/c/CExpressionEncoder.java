...
class CExpressionEncoder implements ExpressionEncoder {
	...
	private class ExpressionVisitor extends Visitor {
		...
		public Expression visitConditionalExpression(GNode node) {
			...
			if (targetType.isPointer()) {
				...
				trueCase = encoding.castToInteger(trueCase, size,
						!CType.isUnsigned(CType.getType(node.getNode(1))));
				falseCase = encoding.castToInteger(falseCase, size,
						!CType.isUnsigned(CType.getType(node.getNode(2))));
			}
		...
		public Expression visitShiftExpression(GNode node) {
			...
			if (">>".equals(shiftOp))
				if (CType.isUnsigned(CType.getType(node.getNode(0))))
					...
					return encoding.signedRshift(coerceToInteger(left),
							coerceToInteger(right));
		...
		public Expression visitAssignmentExpression(GNode node) {
			...
			boolean isUnsigned = CType.isUnsigned(lhsType)
					|| CType.isUnsigned(rhsType);
			...
			if ("/=".equals(assignOp)) {
				...
				if (isUnsigned)
					...
					return encoding.signedDivide(coerceToInteger(left),
							coerceToInteger(right));
			}
			if ("%=".equals(assignOp)) {
				Pair<Expression, Expression> pair = encoding.arithTypeConversion(left,
				...
				if (isUnsigned)
					...
					return encoding.signedRem(coerceToInteger(left),
							coerceToInteger(right));
			}
			if ("|=".equals(assignOp)) {
				Pair<Expression, Expression> pair = encoding.arithTypeConversion(left,
		...
		public Expression visitCastExpression(GNode node) {
			...
			if (targetType.isPointer())
				return encoding.castToPointer(src);
		...
		public Expression visitEqualityExpression(GNode node) {
			...
			if ("==".equals(eqOp))
				return encoding.eq(left, right);
			if ("!=".equals(eqOp))
				return encoding.neq(left, right);

			throw new ExpressionFactoryException("Invalid operation: " + eqOp);
		}

		public List<Expression> visitExpressionList(GNode node) {
			List<Expression> subExprList = Lists
					.newArrayListWithCapacity(node.size());
		...
		public Expression visitRelationalExpression(GNode node) {
			...
			if (lType.isPointer()) {
				...
				if (">".equals(relOp))
					return encoding.pointerGreaterThan(left, right);
				if (">=".equals(relOp))
					return encoding.pointerGreaterThanOrEqual(left, right);
				if ("<".equals(relOp))
					return encoding.pointerLessThan(left, right);
				if ("<=".equals(relOp))
					return encoding.pointerLessThanOrEqual(left, right);
			...
			if (isUnsigned) {
				if (">".equals(relOp))
					return encoding.greaterThan(left, right);
				if (">=".equals(relOp))
					return encoding.greaterThanOrEqual(left, right);
				if ("<".equals(relOp))
					return encoding.lessThan(left, right);
				if ("<=".equals(relOp))
					return encoding.lessThanOrEqual(left, right);

				throw new ExpressionFactoryException("Invalid operation: " + relOp);
			}

			if (">".equals(relOp))
				return encoding.signedGreaterThan(left, right);
			if (">=".equals(relOp))
				return encoding.signedGreaterThanOrEqual(left, right);
			if ("<".equals(relOp))
				return encoding.signedLessThan(left, right);
			if ("<=".equals(relOp))
				return encoding.signedLessThanOrEqual(left, right);
		...
		public Expression visitSubscriptExpression(GNode node) {
			IOUtils.debug().pln("APPROX: Treating pointer as char*");
		...
		public Expression visitDirectComponentSelection(GNode node) {
			...
			long offset = encoding.getCTypeAnalyzer()
					.getOffset(baseType.toStructOrUnion(), fieldName);
		...
		private IRVarInfo createCtrlVar(GNode node) {
			...
			IRVarInfo varInfo = VarInfoFactory
					.createVarInfo(currScope.getQualifiedName(), name, ty);
	...
	private class LvalVisitor extends Visitor {
		...
		public Expression visitSubscriptExpression(GNode node) {
			...
			return pointerPlus(base, index, CType.getType(node.getNode(0)),
	...
	public static CExpressionEncoder create(Mode mode) {
		ExpressionEncoding encoding = mode.getEncoding();
		StateFactory<?> stateFactory = mode.getStateFactory();

		IOUtils.debug()
				.pln("Creating CExpressionEncoder with encoding: " + encoding);
