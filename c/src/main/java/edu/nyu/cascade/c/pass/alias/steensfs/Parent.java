...
class Parent {
  ...
  private final Kind kind;
	...
	private Parent(Kind kind, Set<ECR> parents) {		
		...
		for(ECR parent : parents) {
			if(!((ECR) parent.findRoot()).getType().isStruct())
				IOUtils.err().println("Invalid parent");
		}
	...
	static Parent create(Collection<ECR> ecrs) {
		if(ecrs.isEmpty()) return getBottom();
	...
	static Parent getLUB(Parent p1, Parent p2) {		
		Parent top = getTop();
		if(p1.isTop() || p2.isTop())	return top;
		if(p1.isBottom()) return p2;
		if(p2.isBottom()) return p1;
	 ...
	 * @return
	 */
	static Parent getDiff(Parent p1, Parent p2) {
		if(p1.isBottom() || p2.isTop())	return p1;
		Set<ECR> ecrs = Sets.difference(p1.getECRs(), p2.getECRs());		
		return create(ecrs);
	}
	...
	Set<ECR> getECRs() {
		...
		for(ECR ecr : parents) {
			...
			if (!ecr.getType().isStruct()) 
				IOUtils.out().println("Invalid type");
			ecrs.add(ecr);
		}
		return ecrs;
  }
	...
	Parent removeECR(ECR parent) {
		if(kind.equals(Kind.TOP) || kind.equals(Kind.BOTTOM)) return this;
		
		Set<ECR> ecrs = Sets.newHashSet(parents);
		for(ECR ecr : parents) {
			if(ecr.equals(parent)) {
				ecrs.remove(ecr);
				if(ecrs.isEmpty())	return botInstance;
