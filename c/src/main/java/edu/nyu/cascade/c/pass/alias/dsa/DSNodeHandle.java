...
class DSNodeHandle implements Comparable<DSNodeHandle> {
	...
	/**
	 * Forwarding simulates the union-find algorithm. For DSNode,
	 * ForwardNH is a representative of the equivalence class.
	 * We update the DSNode and Offset in DSNodeHandle.
	 */
	private DSNode HandleForwarding() {
		Preconditions.checkArgument(N.isForwarding()); // Can only be invoked if forwarding!
	...
	/** 
	 * isNull - Check to see if getNode() == null, without going through the trouble
	...
	void setTo(DSNode n, long offset) {
		...
		if (N != null) getNode().NumReferrers--;
		N = n;
		Offset = offset;
		if (N != null) {
			N.NumReferrers++;
	...
	public boolean equals(Object O) {
		if(!(O instanceof DSNodeHandle)) return false;
		DSNodeHandle ONH = (DSNodeHandle) O;
		if(ONH.getNode() != this.getNode()) return false;
