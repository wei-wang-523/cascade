...
class ECRChecker extends Visitor {
	 ...
	 */
	private final Map<Triple<String, String, String>, ECR> ecrMap;

	/**
	 * Store all the ECRs for operation node
	 */
	private final Map<Triple<GNode, String, String>, ECR> opECRMap;
	...
	private class LvalVisitor extends Visitor {
		...
		public ECR visitSimpleDeclarator(GNode node) throws PreProcessorException {
		...
		public ECR visitEnumerator(GNode node) throws PreProcessorException {
			String id = node.getString(0);
		...
		public ECR visitIndirectComponentSelection(GNode node) {
			...
			return getComponent(baseECR, offset);
		}
	...
	ECR getFunctionECR(String functionName) throws PreProcessorException {
	...
	private ECR getComponent(ECR srcECR, long offset) {
		...
		if (locType.isSimple())
			...
			return locType.asStruct().getFieldMap().get(offset);
	}
	...
	private ECR lookupOpECR(GNode opNode) {
		...
		Triple<GNode, String, String> lastKey = Triple.of(opNode, scopeName,
				lastScope);
	...
	private ECR lookup(String id, String scopeName) throws PreProcessorException {
		Iterator<String> scopeItr = CScopeAnalyzer.getScopes().iterator();
		while (scopeItr.hasNext()) {
			String parentScope = scopeItr.next();
		...
		throw new PreProcessorException("ECRChecker cannot find the ECR");
	}
}
