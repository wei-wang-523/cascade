...
import edu.nyu.cascade.c.pass.alias.steensfs.Offset.Kind;
import edu.nyu.cascade.c.pass.alias.steensfs.ValueType.ValueTypeKind;
...
class UnionFindECR {
	...
	void ccjoin(Size rangeSize, ECR e1, ECR e2) {
		...
		if (!Size.isLessThan(rangeSize, size1)) {
			addCCjoin(rangeSize, e1, e2);
			expand(e1); // expand(e1) would call setType(e1, ...) and thus ccjoin(e1,
									// e2)
			return;
		}
	...
	/**
	 * If <code>o</code> is <code>ZERO</code>, add <code>ecr</code> to the
	 * collapse pendings of <code>o</code>, otherwise, collapse <code>ecr</code>
	 * 
	 * @param o
	 * @param ecr
	 */
	...
	ValueType unify(ValueType t1, ValueType t2) {
		...
		case BLANK: {
			...
			default: // case OBJECT:
				return ValueType.object(t2.asObject().getLoc(), t2.asObject().getFunc(),
						size, parent);
			}
		}
		case SIMPLE: {
	 ...
	 * set its type as object type with size Top
	 * 
	 * @param structECR
	 * @param structT
	 */
	...
	/**
	 * Collapse <code>structECR</code> by merge it with all its element ECRs, and
	 * set its type as the object type with structT's size
	 * 
	 * @param structECR
	 * @param structT
	 * @param ECRCache
	...
	/**
	 * Create a pair of location ECR and function ECR, both with bottom type
	 * 
	 * @return
	 */
	...
	void ensureSimObj(ECR e, Size rangeSize) {
		...
		switch (type.getKind()) {
		case BOTTOM: {
			ValueType simType = ValueType.simple(createBottomLocFunc(), rangeSize,
					Parent.getBottom());
			setType(e, simType);
			return;
		}
		...
		case SIMPLE: {
			...
			if (!(Size.isLessThan(rangeSize, size)))
				expand(e);
			return;
		}
		case STRUCT: {
	...
	private void checkStructCollapse(ECR e1, ECR e2) {
		...
		if (t2.isStruct() && (t1.isSimple() || t1.isObject())) {
			collapseStruct(e2, t2.asStruct());
			return;
		}

		if (t1.isStruct() && (t2.isSimple() || t2.isObject())) {
	...
	private ValueType resolveType(ECR root, ValueType unionType,
		...
		if (t1.isStruct() && t2.isObject()) {
			Collection<ECR> elems = t1.asStruct().getFieldMap().asMapOfRanges()
					.values();
