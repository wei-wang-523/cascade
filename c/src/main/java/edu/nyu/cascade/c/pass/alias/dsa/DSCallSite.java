...
/***
 * DSCallSite - Representation of a call site via its call instruction, the
 * DSNode handle for the callee function (or function pointer), and the DSNode
 * handles for the function arguments.
 * 
 * @author Wei
 *
 */
...
class DSCallSite {
	...
	private DSCallSite(Node callSite, Function calleeF, DSNodeHandle calleeN,
			DSNodeHandle retVal, DSNodeHandle varArgNH, List<DSNodeHandle> args,
	...
	DSCallSite(Node callSite, DSNode funcN, DSNodeHandle retVal,
			DSNodeHandle varArgNH, List<DSNodeHandle> args) {
		Preconditions.checkNotNull(callSite); // Null callee node specified for call
																					// site!
		CallSite = callSite;
		CalleeF = null;
	...
	DSCallSite(Node callSite, Function calleeF, DSNodeHandle retVal,
			DSNodeHandle varArgNH, List<DSNodeHandle> args) {
		Preconditions.checkNotNull(callSite); // Null callee node specified for call
																					// site!
		CallSite = callSite;
	 ...
	 * to copy plus a map that specifies how the links should be transformed. This
	 * is useful when moving a call site from one graph to another.
	 */
	static DSCallSite createWithDSNodeMap(DSCallSite FromCall,
			Map<DSNode, DSNodeHandle> NodeMap) {
		...
		return new DSCallSite(Call, CalleeF, CalleeN, RetVal, VarArgVal, CallArgs,
				MappedSites);
	}

	static DSCallSite createWithDSNodeHandleMap(DSCallSite FromCall,
			Map<DSNode, DSNodeHandle> NodeMap) {
		Node Call = FromCall.CallSite;
	 ...
	 * function specified by getCalleeFunc. If not, it is an indirect call to the
	 * node specified by getCalleeNode.
	 */
	...
	int getNumMappedSites() {
		return MappedSites.size();
	}
	...
	Set<Node> getMappedSites() {
		return MappedSites;
	}

	void addPtrArg(DSNodeHandle NH) {
		CallArgs.add(NH);
	}
	...
	void mergeWith(DSCallSite CS) {
		...
		if (CS.getNumPtrArgs() < MinArgs)
			MinArgs = CS.getNumPtrArgs();
	...
	/**
	 * isUnresolvable - Determines if this call has properties that would prevent
	 * it from ever being resolved. Put another way, no amount additional
	 * information will make this callsite resolvable.
	 * 
	 * @return
	 */
