...
public class CfgProcessor {
	...
	/**
	 * Append <code>preCFG</code> to <code>mainCFG</code>, have side-effect on
	 * <code>
	 * mainCFG</code>
	 * 
	 * @param preCFG
	...
	public static void appendPreCFG(IRControlFlowGraph preCFG,
		...
		if (preExit != mainEntry)
			mainCFG.addEdge(preExit, mainEntry);
	...
	private static void replaceBlock(IRControlFlowGraph cfg,
		...
		if (oldBlock == cfg.getEntry())
			cfg.setEntry(newBlock);
		if (oldBlock == cfg.getExit())
			cfg.setExit(newBlock);
	}
	...
	private static boolean mergeBlockIntoPredecessor(IRControlFlowGraph cfg,
		...
		if (!predBBStmts.isEmpty()) {
			...
			if (StatementType.RETURN.equals(lastStmt.getType()))
				return false;
		}

		/*
		 * Traverse successors of the single predecessor, check if all of them are
		 * same of this block. Cannot merge if there are multiple successors.
		 */
		...
		/*
		 * Get rid of unneeded guarded incoming edge (guard as the last assumption
		 * statement of predBB).
		 */
