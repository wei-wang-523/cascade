...
/**
 * Cell-based field-sensitive and context-sensitive alias analysis algorithm
 * (optimized version)
 * 
 * @author Wei
 *
...
public class SteensgaardCFSCS implements IRAliasAnalyzer<ECR> {
	...
	public String displaySnapShot() {
		...
		StringBuilder sb = new StringBuilder().append('\n').append(
				"The result of cell-based field-sensitive and (partial) context-sensitive Steensgaard analysis:\n");
	...
	private String formatECR(Collection<ECR> visited, ECR ecr) {
		...
		case STRUCT: {
			for (ECR field : uf.getType(ecr).asStruct().getFieldMap().asMapOfRanges()
					.values()) {
