...
class ECREncoder extends Visitor {
  ...
  ECR getFunctionECR(String functionName) {
	...
	private ECR createECR(Type type) {
		...
		if(type.isFunction()) return createForFunction(type);
	  ...
	  BlankType varType = ValueType.blank(size, Parent.getBottom());
		...
		ValueType addrType = ValueType.simple(
				varECR, 
				ECR.createBottom(), 
	...
	private ECR createForFunction(Type type) {		
		...
		ValueType varType = ValueType.simple(
				loc, func, size, Parent.getBottom());
		ECR varECR = ECR.create(varType);
		varECR.setOffset(Offset.createZero());
	...
	/**
	 * Get the ECR of <code>opNode</code> in the format as <code>op(leftECR, rightECR)</code>
	 * @param leftECR
	 * @param rightECR
	 * @return
	 */
	private ECR getOpECR(ECR leftECR, ECR rightECR) {
		...
		ValueType resType = ValueType.simple(
				resLoc, ECR.createBottom(), Size.getBot(), Parent.getBottom());
  ...
  /**
	 ...
	 * <code>range</code>, where <code>parent</code>
