...
class VarImpl implements IRVar {
   ...
   * @return
   */
  static VarImpl[] createForStructOrUnionSymbol(String name, Type xtcType, String scopeName) {
  	Preconditions.checkArgument(xtcType.resolve().isStruct() || xtcType.resolve().isUnion());
		
    BlankType regionType = ValueType.blank(Size.createForType(xtcType), Parent.getBottom());
    
  	ECR regECR = ECR.create(regionType);
  	regECR.setOffset(Offset.createZero());
   ...
   * @return
   */
  static VarImpl[] createForArraySymbol(String name, Type xtcType, String scopeName) {
  	Preconditions.checkArgument(xtcType.resolve().isArray());
  	
    BlankType blankType = ValueType.blank(Size.createForType(xtcType), Parent.getBottom());
	...
	static VarImpl createForScalarSymbol(String name, Type xtcType, String scopeName) {
