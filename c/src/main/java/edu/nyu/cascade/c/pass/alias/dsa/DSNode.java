...
abstract class DSNode implements Comparable<DSNode> {
	 ...
	 */
	long getNumReferrers() { return NumReferrers; }

	/** hasNoReferrers - Return true if nothing is pointing to this node at all. */
	boolean hasNoReferrers() { return getNumReferrers() == 0; }
	
	DSGraph getParentGraph() { return ParentGraph; }
	 ...
	 * to, if any.
	 */	
	DSNode getForwardNode() { return ForwardNH.getNode(); }

	/** isForwarding - Return true if this node is forwarding to another. */
	boolean isForwarding() { return !ForwardNH.isNull(); }
	 ...
	 * to accommodate NewTy at the given offset. This is useful for
	...
	boolean hasLink(long Offset) {
		Preconditions.checkArgument(Offset < getSize()); // Link index is out of range!
	 ...
	 */
	boolean isNodeCompletelyFolded() { return isCollapsedNode(); }
	...
	void addFunction(Function F) { addGlobal(F); }
	...
	void mergeGlobals(DSNode RHS) { Globals.addAll(RHS.Globals); }
	
	void clearGlobals() { Globals.clear(); }
	
	boolean isEmptyGlobals() { return Globals.isEmpty(); }
	int numGlobals() { return Globals.size(); }
	 ...
	 * (keeping both for compact with existing clients)
	 ...
	 */
	int getNodeFlags() { return NodeType & ~NodeTy.DeadNode.value(); }
	...
	boolean isAllocaNode()     { return (NodeType & NodeTy.AllocaNode.value()) != 0;		}
	boolean isHeapNode()       { return (NodeType & NodeTy.HeapNode.value())!= 0;     	}
	boolean isGlobalNode()     { return (NodeType & NodeTy.GlobalNode.value())!= 0;      	}
	boolean isExternFuncNode() { return (NodeType & NodeTy.ExternFuncNode.value())!= 0;  	}
	boolean isUnknownNode()    { return (NodeType & NodeTy.UnknownNode.value())!= 0;      }
	boolean isModifiedNode()   { return (NodeType & NodeTy.ModifiedNode.value())!= 0;     }
	boolean isReadNode()       { return (NodeType & NodeTy.ReadNode.value())!= 0;     	}
	boolean isArrayNode()      { return (NodeType & NodeTy.ArrayNode.value())!= 0;      	}
	boolean isCollapsedNode()  { return (NodeType & NodeTy.CollapsedNode.value())!= 0;    }
	boolean isIncompleteNode() { return (NodeType & NodeTy.IncompleteNode.value())!= 0;   }
	boolean isCompleteNode()   { return !isIncompleteNode();      }
	boolean isDeadNode()       { return (NodeType & NodeTy.DeadNode.value())!= 0;     	}
	boolean isExternalNode()   { return (NodeType & NodeTy.ExternalNode.value())!= 0;     }
	boolean isIntToPtrNode()   { return (NodeType & NodeTy.IntToPtrNode.value())!= 0;     }
	boolean isPtrToIntNode()   { return (NodeType & NodeTy.PtrToIntNode.value())!= 0;     }
	boolean isVAStartNode()    { return (NodeType & NodeTy.VAStartNode.value())!= 0;      }
	
	DSNode setAllocaMarker()     { NodeType |= NodeTy.AllocaNode.value();     return this; }
	DSNode setHeapMarker()       { NodeType |= NodeTy.HeapNode.value();       return this; }
	DSNode setGlobalMarker()     { NodeType |= NodeTy.GlobalNode.value();     return this; }
	DSNode setExternFuncMarker() { NodeType |= NodeTy.ExternFuncNode.value(); return this; }
	DSNode setExternGlobalMarker() { NodeType |= NodeTy.ExternGlobalNode.value(); return this; }
	DSNode setUnknownMarker()    { NodeType |= NodeTy.UnknownNode.value();    return this; }
	DSNode setModifiedMarker()   { NodeType |= NodeTy.ModifiedNode.value();   return this; }
	DSNode setReadMarker()       { NodeType |= NodeTy.ReadNode.value();       return this; }
	DSNode setArrayMarker()      { NodeType |= NodeTy.ArrayNode.value();      return this; }
	DSNode setCollapsedMarker()  { NodeType |= NodeTy.CollapsedNode.value();  return this; }
	DSNode setIncompleteMarker() { NodeType |= NodeTy.IncompleteNode.value(); return this; }
	DSNode setExternalMarker()   { NodeType |= NodeTy.ExternalNode.value();   return this; }
	DSNode setIntToPtrMarker()   { NodeType |= NodeTy.IntToPtrNode.value();   return this; }
	DSNode setPtrToIntMarker()   { NodeType |= NodeTy.PtrToIntNode.value();   return this; }
	DSNode setVAStartMarker()    { NodeType |= NodeTy.VAStartNode.value();    return this; }
	...
	SortedSet<GlobalValue> getGlobals() { return Globals; }
}
