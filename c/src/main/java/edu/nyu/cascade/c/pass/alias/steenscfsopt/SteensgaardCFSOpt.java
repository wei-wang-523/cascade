...
/**
 * Cell-based field-sensitive alias analysis algorithm (optimized version)
 * 
 * @author Wei
 *
 */
public class SteensgaardCFSOpt implements IRAliasAnalyzer<ECR> {	
  ...
  private SteensgaardCFSOpt (SymbolTable symbolTable) {
    uf = UnionFindECR.create();
    ecrEncoder = ECREncoder.create(uf, symbolTable); 
    this.symbolTable = symbolTable;
  }
  
  public static SteensgaardCFSOpt create(SymbolTable symbolTable) {
    return new SteensgaardCFSOpt(symbolTable);
  }
			...
			for(IRBasicBlock block : topologicSeq) {			
				...
				for(IREdge<?> outgoing : globalCFG.getOutgoingEdges(block)) {
					if(null != outgoing.getGuard()) {
						...
						IOUtils.debug().pln("Preprocess: " + guardNode.getLocation() + ": " + guard);
