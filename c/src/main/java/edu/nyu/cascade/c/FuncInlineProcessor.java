...
public class FuncInlineProcessor<T> {
	...
	void inlineMalloc(Map<Node, IRControlFlowGraph> cfgs,
		...
		while (!worklist.isEmpty()) {
			...
			if (ReservedFunction.MALLOC.equals(callee))
				continue;
	...
	private boolean functionInlineCFGWithLevel(IRControlFlowGraph cfg,
		...
		if (Iterables.isEmpty(funcCallBlocks))
			return false;
	...
	/**
	 * Check if <code>funcNode</code> is function call not function pointer call
	 * 
	 * @param funcNode
	 * @return
	 */
	private boolean isFunctionCall(Node funcNode) {
		...
		/*
		 * Cannot pick funcName from funcNode, since funcNode is at least not
		 * PrimaryIdentifier, thus not function global-defined
		 */
		if (null == funcName)
			return false;
		...
		if (!symbolTable.isDefined(funcName))
			return false;
	...
	/**
	 * Get the argument passing statements of function call nested in the
	 ...
	 * <code>stmt</code> 0) function enter statement 1) arguments passing 2)
	...
	/**
	 * Function inline for every the function call statement in
	 * <code>funcCallBlock</code> into <code>CFG</code>, while <code>level</code>
	 * used to process function inline for the function call statements in the
	 * function CFG recursively.
	 * 
	 * @param CFG
	 * @param funcCallBlock
	 * @param level
	...
	private boolean functionInlineBlock(IRControlFlowGraph CFG,
		...
		do {
			...
			/*
			 * Isolate funcCall statement in nextBlock, restBlock contains the
			...
			if (currBlock.equals(CFG.getExit()))
				CFG.setExit(restBlock);
	...
	/**
	 * Function inline funcCFG for every the function call statement of the
	 * function in <code>funcCallBlock</code> into <code>CFG</code>.
	 * 
	 * @param CFG
	 * @param funcCallBlock
	 * @param funcName
	...
	private Collection<IRControlFlowGraph> lookupFuncCFG(Node funcNode,
		...
		for (IRVar var : funcVars) {
			...
			if (composeType.isError())
				continue;

			IRControlFlowGraph funcCFG = cfgs.get(name);
			if (null != funcCFG)
				funcCFGs.add(funcCFG);
		}
