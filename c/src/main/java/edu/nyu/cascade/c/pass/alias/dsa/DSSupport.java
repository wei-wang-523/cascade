...
class DSSupport {
	...
	enum NodeTy {
		...
		Composition(AllocaNode.value() | HeapNode.value() | GlobalNode.value()
				| UnknownNode.value());
	...
	enum MarkIncompleteFlags {
		MarkFormalArgs(1), IgnoreFormalArgs(0), IgnoreGlobals(
				2), MarkGlobalIncomplete(0), MarkVAStart(4);
	...
	enum ComputeExternalFlags {
		MarkFormalsExternal(1), DontMarkFormalsExternal(0), ProcessCallSites(
				2), IgnoreCallSites(0), ResetExternal(4), DontResetExternal(0);
	...
	static void MergeNodes(DSNodeHandle CurNodeH, DSNodeHandle NH) {
		...
		// If we are merging a node with a completely folded node, then both nodes
		// are
