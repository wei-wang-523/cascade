...
public class TraceGraphMLBuilder {
	...
	public void dumpXmlTrace(JAXBElement<?> traceGraphML,
	...
	private void analyzeIRMLNode(IRTraceNode traceNode, MLNode preMLNode) {
		...
		for (IRStatement stmt : traceNode.getStatements()) {
			...
			case INIT: {
				...
				StringBuilder sb = new StringBuilder();
				sb.append(stmt.getOperand(0)).append(" == ").append('(').append(
						traceExpr).append(')');
				...
				String qualifiedScopeName = CType.getScopeName(stmt.getOperand(0)
						.getSourceNode());
			...
			case CALL: {
				if (!CType.getType(srcNode).resolve().isVoid()) {
					Expression traceExpr = traceNode.getTraceExpr(stmt);
					StringBuilder sb = new StringBuilder();
					sb.append(stmt.getOperand(1)).append(" == ").append('(').append(
							traceExpr).append(')');
					edge.setAssumption(sb.toString());
				}
				break;
	...
	private JAXBElement<GraphmlType> build(String srcFile) {
		...
		keys.add(getKeyType("assumption", "assumption", KeyForType.EDGE,
				KeyTypeType.STRING));
		keys.add(getKeyType("assumption.scope", "assumption.scope", KeyForType.EDGE,
				KeyTypeType.STRING));
		keys.add(getKeyType("sourcecode", "sourcecode", KeyForType.EDGE,
				KeyTypeType.STRING));
		keys.add(getKeyType("sourcecodeLanguage", "sourcecodelang",
				KeyForType.GRAPH, KeyTypeType.STRING));
		// keys.add(getKeyType("tokenSet", "tokens", KeyForType.EDGE,
		// KeyTypeType.STRING));
		// keys.add(getKeyType("originTokenSet", "origintokens", KeyForType.EDGE,
		// KeyTypeType.STRING));
		keys.add(getKeyType("negativeCase", "negated", KeyForType.EDGE,
				KeyTypeType.STRING, String.valueOf(false)));
		keys.add(getKeyType("startline", "startline", KeyForType.EDGE,
				KeyTypeType.INT));
		keys.add(getKeyType("originFileName", "originfile", KeyForType.EDGE,
				KeyTypeType.STRING, "&lt;command-line&gt;"));
		keys.add(getKeyType("control", "control", KeyForType.EDGE,
				KeyTypeType.STRING));
		keys.add(getKeyType("nodeType", "nodetype", KeyForType.NODE,
				KeyTypeType.STRING, "path"));
		keys.add(getKeyType("isFrontierNode", "frontier", KeyForType.NODE,
				KeyTypeType.BOOLEAN, String.valueOf(false)));
		keys.add(getKeyType("isViolationNode", "violation", KeyForType.NODE,
				KeyTypeType.BOOLEAN, String.valueOf(false)));
		keys.add(getKeyType("isEntryNode", "entry", KeyForType.NODE,
				KeyTypeType.BOOLEAN, String.valueOf(false)));
		keys.add(getKeyType("isSinkNode", "sink", KeyForType.NODE,
				KeyTypeType.BOOLEAN, String.valueOf(false)));
		keys.add(getKeyType("enterFunction", "enterFunction", KeyForType.EDGE,
				KeyTypeType.STRING));
		keys.add(getKeyType("returnFromFunction", "returnFrom", KeyForType.EDGE,
				KeyTypeType.STRING));
	...
	private KeyType getKeyType(String attrName, String id, KeyForType forWhat,
	...
	private KeyType getKeyType(String attrName, String id, KeyForType forWhat,
			KeyTypeType keyTypeType, String defaultVal) {
		KeyType keyType = objectFactory.createKeyType();
		keyType.setAttrName(attrName);
		keyType.setAttrType(keyTypeType);
		keyType.setFor(forWhat);
