...
public class StmtBasedFormulaEncoder implements FormulaEncoder {
	...
	private class LoopUnrollMerger<T> {
		private final Multimap<IREdge<?>, T> exitMultiSrcState = LinkedHashMultimap
				.create();
		 ...
		 */
		Map<IREdge<?>, T> getExitEdgeStateMap(
	...
	private final Map<Loop, LoopCountDownLatch> loopCountDownLatchMap = Maps
			.newHashMap();
	private final Map<Expression, Collection<? extends IREdge<? extends IRBasicBlock>>> exitUnrollMap = Maps
			.newLinkedHashMap();
	private final Map<Expression, Collection<? extends IREdge<? extends IRBasicBlock>>> keepUnrollMap = Maps
			.newLinkedHashMap();

	private StmtBasedFormulaEncoder(PathEncoding pathEncoding,
	...
	public static StmtBasedFormulaEncoder create(PathEncoding encoding,
			TraceFactory traceFactory) {
	...
	public void encode(final IRControlFlowGraph cfg, LoopInfo loopInfo)
		...
		MemLeakCheckStrategy<StateExpression> memLeakCheckStrategy = new MemLeakCheckStrategy<StateExpression>() {
			...
			public void apply(StateExpression state) throws PathFactoryException {
				...
				if (Preferences.isSet(Preferences.OPTION_TWOROUND_MEMCHECK)) {
					if (!Preferences.isSet(Preferences.OPTION_MEMTRACK))
						return;
					Expression memTracker = state.getMemTracker().simplify();
					BooleanExpression assumption = stateFactory.stateToBoolean(state)
		...
		statePropagateDFS(cfg, loopInfo, blockStrategy, edgeStrategy, mergeStrategy,
				traceEncodeStrategy, memLeakCheckStrategy, checkExitUnroll,
				checkKeepUnroll, pathEncoding.emptyState());
	...
	public IRTraceNode getErrorTrace(IRControlFlowGraph cfg) {
		...
		traceFactory.filterCounterExample(pathEncoding.getExpressionManager(),
				entryNode);
		return entryNode;
	}

	@Override
	public void checkReach(final IRControlFlowGraph cfg, LoopInfo loopInfo,
	...
	public Pair<Boolean, StateExpression> encodeBlock(TraceFactory factory,
		...
		for (IRStatement stmt : block.getStatements()) {
			...
			if (stmt.getType().equals(StatementType.ASSERT)) {
				...
				if (succeed)
					checkFeasibility(currState);

				if (!succeed)
					return Pair.of(succeed, currState); // return false to interrupt
																							// encoding
			}
		}
		return Pair.of(true, currState);
	...
	private <T> boolean statePropagateDFSInLoop(IRControlFlowGraph cfg,
			LoopInfo loopInfo, BlockEncodingStrategy<T> blockStrategy,
			EdgeEncodingStrategy<T> edgeStrategy,
			PhiNodeResolveStrategy<T> mergeStrategy,
			TraceEncodeStrategy traceEncodeStrategy,
			final Map<IREdge<?>, T> edgeSrcStateMap, Loop loop,
			boolean checkExitUnroll, boolean checkKeepUnroll, T preLoopHeaderState)
			throws PathFactoryException {
		...
		while (!propagateWorkList.isEmpty()) {
			...
			if (block == loop.getHeader() && countDownLatch.notCountDownYet()) {
				...
				if (countDownLatch.stop()) {
					...
					if (!blocksInExitRound.contains(block))
						continue;
				}
			...
			if (isNestLoopHeader) { // nested loop header, recursive call
				...
				boolean funcQueryResult = statePropagateDFSInLoop(cfg, loopInfo,
						blockStrategy, edgeStrategy, mergeStrategy, traceEncodeStrategy,
						edgeSrcStateMap, nestLoop, checkExitUnroll, checkKeepUnroll,
						preState);
				if (!funcQueryResult)
					return funcQueryResult;
				succEdges = nestLoop.getExitEdges();

				if (checkExitUnroll)
					addExitUnrollAssumptions(succEdges, edgeSrcStateMap);
				if (checkKeepUnroll)
					addKeepUnrollAssumptions(succEdges, edgeSrcStateMap);

			} else {
				traceEncodeStrategy.encode(block);
				Pair<Boolean, T> stateAndQueryResult = blockStrategy.apply(block,
						preState);
			...
			for (IREdge<?> succEdge : succEdges) {
				...
				if (succ == loop.getHeader()) {
					...
					/*
					 * Do not push loop header to work list until all backEdges are
					 * encoded
					 */
					...
					if (isReadyToWork) {
						countDownLatch.countDown(); // count-down the latch if all
																				// back-edges are encoded
						propagateWorkList.push(succ);
					}

				} else {
					boolean isSuccNestLoopHeader = loopInfo.getInnerLoopMap().containsKey(
							succ) && succ == loopInfo.getInnerLoopMap().get(succ).getHeader();

					if (isSuccNestLoopHeader) {
						/*
						 * Do not add nested loop header to work list until all its incoming
						 * edges except back-edges are encoded and stored in
						 * edge-src-state-map
						 */
						...
						boolean readyToWork = Iterables.all(cfg.getIncomingEdges(succ),
								new Predicate<IREdge<?>>() {
									...
									public boolean apply(IREdge<?> edge) {
										...
										/*
										 * clean the nest back edges if it's encoded (maybe in the
										 * last time encode the loop) back edges cannot be encoded
										 * before encode the nest loop
										 */
						...
						/*
						 * Do not push successor to work list until all its incoming edges
						 * are encoded and stored in edge-src-state-map
						 */
	...
	<T> void statePropagateDFS(IRControlFlowGraph cfg, LoopInfo loopInfo,
			BlockEncodingStrategy<T> blockStrategy,
			EdgeEncodingStrategy<T> edgeStrategy,
			PhiNodeResolveStrategy<T> mergeStrategy,
			TraceEncodeStrategy traceEncodeStrategy,
			MemLeakCheckStrategy<T> memLeakCheckStrategy, boolean checkExitUnroll,
			boolean checkKeepUnroll, T initState) throws PathFactoryException {
	 ...
	 * @throws PathFactoryException
	 */
	private StateExpression encodeStatement(IRStatement stmt,
			StateExpression preCondition) throws PathFactoryException {
		...
		for (Entry<String, BooleanExpression> entry : postCond.getAssertions()
				.entries()) {
	...
	private <T> Pair<Boolean, T> getMergedPreState(
		...
		for (IREdge<? extends IRBasicBlock> incoming : incomingEdges) {
			...
			/*
			 * Get the src-state, and remove the edge for future propagation in next
			 * round loop-unrolling
			 */
	...
	private boolean isFilter(Map<IREdge<?>, StateExpression> edgeSrcStateMap,
			IREdge<?> edge) throws PathFactoryException {
		if (edge.getGuard() == null)
			return false;
		if (!edgeSrcStateMap.containsKey(edge))
			return false;
	...
	private void filterEdge(IRControlFlowGraph cfg,
			Collection<IREdge<?>> filterEdges, IREdge<?> edge) {
		if (!filterEdges.add(edge))
			return; // not fresh edge

		IRBasicBlock target = edge.getTarget();
		Collection<? extends IREdge<?>> incomings = cfg.getIncomingEdges(target);
		if (!filterEdges.containsAll(incomings))
			return;
	...
	private void filterEdgeInLoop(IRControlFlowGraph cfg,
			Collection<IREdge<?>> filterEdges, Loop loop, IREdge<?> edge) {
		if (!filterEdges.add(edge))
			return; // not fresh edge
		if (loop.getExitEdges().contains(edge))
			return; // do not add edges outside loop
	...
	<T> void statePropagateDFSFilter(IRControlFlowGraph cfg, LoopInfo loopInfo,
			BlockEncodingStrategy<T> blockStrategy,
			EdgeEncodingStrategy<T> edgeStrategy,
			PhiNodeResolveStrategy<T> mergeStrategy,
			TraceEncodeStrategy traceEncodeStrategy,
			MemLeakCheckStrategy<T> memLeakCheckStrategy,
			EdgeFilterStrategy<T> edgeFilterStrategy, boolean checkExitUnroll,
			boolean checkKeepUnroll, T initState) throws PathFactoryException {
		...
		while (!propagateWorkList.isEmpty()) {
			...
			if (isLoopHeader) { // loop header, call propagate state in loop
				Loop nestLoop = loopInfo.getInnerLoopMap().get(block);
				boolean funcQueryResult = statePropagateDFSInLoopFilter(cfg, loopInfo,
						blockStrategy, edgeStrategy, mergeStrategy, traceEncodeStrategy,
						edgeFilterStrategy, edgeSrcStateMap, filteredEdges, nestLoop,
						checkExitUnroll, checkKeepUnroll, preState);

				if (!funcQueryResult)
					return;

				for (IREdge<?> exitEdge : nestLoop.getExitEdges()) {
					if (!filteredEdges.contains(exitEdge))
						succEdges.add(exitEdge);
				}
			...
			for (IREdge<?> succEdge : succEdges) {
				if (edgeFilterStrategy.apply(edgeSrcStateMap, succEdge)) {
					filterEdge(cfg, filteredEdges, succEdge);
					continue;
				}
	...
	private <T> boolean statePropagateDFSInLoopFilter(IRControlFlowGraph cfg,
			...
			final Map<IREdge<?>, T> edgeSrcStateMap,
			final Collection<IREdge<?>> filteredEdges, Loop loop,
			boolean checkExitUnroll, boolean checkKeepUnroll, T preLoopHeaderState)
			throws PathFactoryException {
		Collection<IRBasicBlock> blocksInExitRound = loop.getBlocksInExitRound();
		...
		while (!propagateWorkList.isEmpty()) {
			...
			if (isNestLoopHeader) { // nested loop header, recursive call
				...
				boolean funcQueryResult = statePropagateDFSInLoopFilter(cfg, loopInfo,
						blockStrategy, edgeStrategy, mergeStrategy, traceEncodeStrategy,
						edgeFilterStrategy, edgeSrcStateMap, loopFilterEdges, nestLoop,
						checkExitUnroll, checkKeepUnroll, preState);
				if (!funcQueryResult)
					return funcQueryResult;

				for (IREdge<?> exitEdge : nestLoop.getExitEdges()) {
					if (edgeSrcStateMap.containsKey(exitEdge))
						succEdges.add(exitEdge);
				}
			...
			for (IREdge<?> succEdge : succEdges) {
				...
				if (edgeFilterStrategy.apply(edgeSrcStateMap, succEdge)) {
					filterEdgeInLoop(cfg, loopFilterEdges, loop, succEdge);
					continue;
				}
		...
		for (IREdge<?> exitEdge : loop.getExitEdges()) {
			if (!loopFilterEdges.contains(exitEdge))
				continue;
