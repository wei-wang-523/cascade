...
class CellEncoder extends Visitor {
	...
	/**
	 * Store cells created for declared variables, functions and enumerators with
	 * their references names (variable names) and scope names.
	 */
	private final Map<Pair<String, String>, Cell> leftCellMap = Maps.newHashMap();
	/**
	 * Cache all the cell created for operation node.
	 */
	private final Map<Pair<GNode, String>, Cell> opCellMap = Maps.newHashMap();
	...
	private class LvalVisitor extends Visitor {
		...
		/**
		 * Both l-cell and r-cell of declared variable are created. Store (r-cell,
		 * l-cell) into the addrCellMap. Store r-cell and the variable's identifier
		 * and scopeName into the varCellMap. Return the l-cell of the declared
		 * variable.
		 * 
		 * @param node
		 * @return
		...
		/**
		 * For enumerator, just create a r-cell (no l-cell). Store r-cell and the
		 * enumerator's identifier and scopeName into varCellMap. Note that no
		 * l-cell is created in this case since enumerator is a constant value not
		 * variable.
		 * 
		 * @param node
		 * @return
		...
		/**
		 * Valid array access if baseType is an array type, idxType is with constant
		 * and the constant value is within the size of the array type.
		 */
		private boolean isValidArrayAccess(Type baseType, Type idxType) {
			if (!baseType.resolve().isArray())
				return false;
			if (!idxType.hasConstant())
				return false;
	 ...
	 * exists in the CFPG, return the field. Otherwise, create a field cell and
	 * update CFPG with a new contains edge. Return the cell points-to the field
	 * cell.
	 * 
