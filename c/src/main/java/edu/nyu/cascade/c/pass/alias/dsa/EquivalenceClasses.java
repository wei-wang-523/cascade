...
class EquivalenceClasses<ElemTy> {
	 ...
	 * Each of these represents a relation for a value. First it stores the value
	 * itself, which provides the ordering that the set queries. Next, it provides
	 * a "next pointer", which is used to enumerate all of the elements in the
	 * unioned set. Finally, it defines either a "end of list pointer" or
	 * "leader pointer" depending on whether the value itself is a leader. A
	 * "leader pointer" points to the node that is the leader for this element, if
	 * the node is not a leader. A "end of list pointer" points to the last node
	 * in the list of members of this list. Whether or not a node is a leader is
	 * determined by a bit stolen from one of the pointers.
	 */
	class ECValue {
		private boolean IsLeader;
		// Leader - if it is leader, then Leader is the "end of list pointer";
		// otherwise,
		...
		ECValue getEndOfList() {
			Preconditions.checkArgument(isLeader()); // Cannot get the end of a list
																								// for a non-leader!
			return Leader;
		}

		void setNext(ECValue NewNext) {
			Preconditions.checkArgument(getNext() == null); // Already has a next
																											// pointer!
			Next = NewNext;
		}
	};
	...
	boolean isEmpty() {
		return TheMapping.isEmpty();
	}

	/**
	 * getLeaderValue - Return the leader for the specified value that is in the
	 * set. It is an error to call this method for a value that is not yet in the
	 * set. For that, call getOrInsertLeaderValue(V).
	 */
	 ...
	 * in the set. If the member is not in the set, it is inserted, then returned.
	 */
	...
	/**
	 * getNumClasses - Return the number of equivalence classes in this set. Note
	 ...
	 * equivalence class it is in. This does the path-compression part that makes
	 * union-find "union findy". This returns null if the value is not in the
	 * equivalence class.
	 */
