...
public final class LocalDataStructureImpl extends DataStructuresImpl {
	...
	@Override
	public boolean runOnModule(SymbolTable symTbl, IRControlFlowGraph globalCFG,
		...
		GlobalsGraph.markIncompleteNodes(
				DSSupport.MarkIncompleteFlags.MarkFormalArgs.value()
						| DSSupport.MarkIncompleteFlags.IgnoreGlobals.value());
		GlobalsGraph.computeExternalFlags(
	...
	class GraphBuilder {
		...
		/**
		 * If Ty is a pointer type, return the pointed DSNodeHandle of PtrNH;
		 * otherwise, return the PtrNH itself.
		 */
		...
		private DSNodeHandle fieldSelect(DSNodeHandle NodeH, Type StructTy,
				String FieldName) {
		...
		private DSNodeHandle getElemPtrNH(DSNodeHandle ptrNH, Type ptrTy,
				Type idxTy, boolean isPlus) {
			...
			if (!idxTy.hasConstant()) {
				...
				if (NodeH.getOffset() != 0 || (!CurTy.isArray() && (NodeH.getNode()
						.getSize() != CType.getInstance().getSize(CurTy)))) {
		...
		private void ensureSafeIndexAccess(DSNodeHandle NodeH, Type ElemTy) {
			...
			if (NodeH.getOffset() != 0 || (!ElemTy.resolve().isArray() && NodeH
					.getNode().getSize() != CType.getInstance().getSize(ElemTy))) {
				NodeH.getNode().foldNodeCompletely();
				NodeH.getNode();
			}
		}
		...
		private class RvalVisitor extends Visitor {
			...
			/**
			 * For all other instruction types, if we have any arguments that are of
			 * pointer type, make them have unknown composition bits, and merge the
			 * nodes together.
			 */
			@SuppressWarnings("unused")
			public DSNodeHandle visitAdditiveExpression(GNode node) {
		...
		GraphBuilder(IRControlFlowGraph CFG, DSGraph g) {
			...
			if (!g.getScalarMap().getGlobalSet().isEmpty()) {
				ReachabilityCloner RC = new ReachabilityCloner(g, g.getGlobalsGraph(),
						0, true);
		...
		private DSNodeHandle getLink(final DSNodeHandle NH, int LinkNo) {
			...
			if (Link.isNull()) {
				// If the link hasn't been created yet, make and return a new shadow
				// node
