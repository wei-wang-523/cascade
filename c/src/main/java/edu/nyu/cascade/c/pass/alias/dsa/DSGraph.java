...
abstract class DSGraph {
	...
	DSGraph(EquivalenceClasses<GlobalValue> ECs, SuperSet<Type> tss, DSGraph GG) {
		GlobalsGraph = GG;
		UseAuxCalls = false;
	...
	DSGraph(DSGraph DSG, EquivalenceClasses<GlobalValue> ECs, SuperSet<Type> tss,
			int CloneFlags) {
		GlobalsGraph = null;
		ScalarMap = new DSScalarMap(ECs);
		NodeMap = new DSNodeMap();
	...
	/// graph...
	///
	List<DSCallSite> getFunctionCalls() {
	...
	/// have been run.
	///
	List<DSCallSite> getAuxFunctionCalls() {
		return AuxFunctionCalls;
	}

	// addAuxFunctionCall - Add a call site to the AuxFunctionCallList
	void addAuxFunctionCall(DSCallSite D) {
		AuxFunctionCalls.add(D);
	}

	abstract void buildCallGraph(DSCallGraph DCG,
			Collection<Function> GlobalFunctionList, boolean filter);
	...
	DSNodeHandle getNodeForValue(Value V) {
		return ScalarMap.get(V);
	}

	boolean hasNodeForValue(Value V) {
		return ScalarMap.find(V) != null;
	}
	...
	Map<Function, DSNodeHandle> getReturnNodes() {
		return ReturnNodes;
	}
	...
	Map<Function, DSNodeHandle> getVANodes() {
		return VANodes;
	}
	 ...
	 * this graph, then clearing the RHS graph. Instead of performing this as two
	 * separate operations, do it as a single, much faster, one.
	 */
	...
	///
	abstract void mergeInGraph(DSCallSite CS, Function F, DSGraph Graph,
			int CloneFlags);
	...
	void AssertNodeInGraph(DSNode N) {
		assert (N == null || N
				.getParentGraph() == this) : "AssertNodeInGraph: Node is not in graph!";
	}
	...
	public void format(Printer printer) {
		printer.pln("Graph for: " + getFunctionNames()).incr();
		...
		for (Entry<Pair<Node, String>, DSNodeHandle> entry : NodeMap.getNodeMap()
				.entrySet()) {
