 ...
 * through the program).
 */
class CopyOfRunMergeProcessor implements RunProcessor {
  
  public CopyOfRunMergeProcessor(Map<File, CSymbolTable> symbolTables,
...
/**  
  ...
  private Node findFuncDeclareNode (IRStatement stmt) throws RunProcessorException {
    ...
    Node res = functions.get(funcName);
    if(res == null) {
      IOUtils.err().println("Function " + funcName + " is not defined.");
    } 
    return res;
  }
  
  /** Decide the function declare Node for the function call statement. */
  private boolean isDeclaredFuncCall (IRStatement stmt) throws RunProcessorException {
...
//  }
  ...
  private List<IRStatement> assignArgToParam(CSymbolTable symbolTable, 
    ...
    if(defNode == null)     return Lists.newArrayList();
  ...
  private Graph getGraphForAssignCallStmt(CSymbolTable symbolTable, 
    ...
    if(funcGraph == null)	{
    	IOUtils.err().println("Failed to build graph for " + rhsStmt);
    	return null;
    }
...
//      symbolTable.setScope(currScope);
    ...
    if(pairs.isEmpty())
    	throw new IllegalArgumentException("Cannot pick any statement for " + stmt);
