...
/**
 * DSScalarMap - An instance of this class is used to keep track of all of which
 * DSNode each scalar in a function points to. This is specialized to keep track
 * of globals with nodes in the function, and to keep track of the unique
 * DSNodeHandle being used by the scalar map.
 * 
 * This class is crucial to the efficiency of DSA with some large SCC's. In
 * these cases, the cost of iterating over the scalar map dominates the cost of
 * DSA. In all of these cases, the DSA phase is really trying to identify
...
class DSScalarMap {
	...
	Map<Value, DSNodeHandle> getValueMap() {
		return ValueMap;
	}
	...
	void spliceFrom(DSScalarMap RHS) {
		...
		if (ValueMap.isEmpty()) {
			...
			for (Entry<Value, DSNodeHandle> entry : RHS.ValueMap.entrySet()) {
				...
				DSNodeHandle NH = ValueMap.containsKey(V) ? ValueMap.get(V)
						: new DSNodeHandle();
				ValueMap.put(V, NH);
	...
	private DSNodeHandle AddGlobal(GlobalValue GV) {
		...
		Preconditions.checkArgument(!ValueMap.containsKey(GV)); // "GV already
																														// exists!"
