...
public class SteensStatsTest {
	...
	public SteensStatsTest(File file) {
	...
	public void test() throws ParseException, IOException {
		...
		Steensgaard fi = Steensgaard.create(symbolTable);
		fi.analysis(globalCFG, CFGs);
		...
		for (ECR ecr : aliasMap.keySet()) {
			...
			if (aliasGroup.size() <= 1)
				continue;
			debugPrinter.p(ecr.getId()).p(':').p(ecr.getType().toString());
