...
/* Error trance: error with line 767 if(rootd == 0) instead of (rootd == 1)
 * resolved = '/x' --> rootd == 0, wbuf = "x";
 *    strlen(resolved) = 2;
 *    strlen(resolved) + strlen(wbuf) + rootd + 1 <= MAXPATHLEN
 *    strlen(wbuf) <= MAXPATHLEN - 3 = 1;
 *       i. strcat(resolved, "/") --> resolved = "/x/";
 *       ii.strcat(resolved, wbuf) --> resolved = "/x/" + wbuf = "/x/x"
