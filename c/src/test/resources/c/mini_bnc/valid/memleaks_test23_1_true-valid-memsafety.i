...
//  int flag = hid_parser_main(parser, &item);
 int flag = dispatch_type[0](parser, &item);
 free(parser);
 return 0;
}
...
int probe_23(struct hid_device *dev) {
 int ret = - -3;
 struct hid_report_enum *p = (struct hid_report_enum *)malloc(sizeof(struct hid_report_enum));
