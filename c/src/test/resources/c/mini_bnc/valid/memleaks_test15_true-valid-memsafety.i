...
void _entry_point(void) {
 struct ldv_hid_device *hdev = (struct ldv_hid_device *)
 	malloc(sizeof(struct ldv_hid_device));
 struct ldv_usb_interface *intf = (struct ldv_usb_interface*)
 	malloc(sizeof(struct ldv_usb_interface));
 intf->cur_altsetting = (struct ldv_usb_host_interface *)
 		      malloc(sizeof(struct ldv_usb_host_interface));

 intf->cur_altsetting->desc.bInterfaceProtocol = (char)__VERIFIER_nondet_int();
 hdev->dev.parent = &intf->dev;

 int retval = ldv_arvo_init_specials(hdev);
