...
public class TheoremProverFactory {
	 ...
	 * {@link TheoremProver} instances.
	 * 
	 * @see java.util.ServiceLoader
	 */
	/*
	 * public static void discover() { discoverCalled = true;
	 * 
	 * Give preference to the user-supplied plugins dir File pluginsDir =
	 * Preferences.getFile(Preferences.OPTION_PLUGINS_DIRECTORY);
	 * 
	 * if (pluginsDir != null && !pluginsDir.canRead()) { IOUtils.err().println(
	 * "Can't read plugins directory: " + pluginsDir); pluginsDir = null; }
	 * 
	 * If no user-supplied dir, use default if (pluginsDir == null) { pluginsDir =
	 * DEFAULT_PLUGINS_DIRECTORY; if( !pluginsDir.canRead() ) {
	 * IOUtils.err().println("Can't read plugins directory: " + pluginsDir);
	 * pluginsDir = null; } }
	 * 
	 * ClassLoader classLoader = ClassLoader.getSystemClassLoader();
	 * 
	 * if( pluginsDir != null ) { IOUtils.debug().pln("Plugins dir: " +
	 * pluginsDir);
	 * 
	 * Get a list of the JARs in the plugins dir File[] jarFiles =
	 * pluginsDir.listFiles(new FileFilter() {
	 * 
	 * @Override public boolean accept(File pathname) { return
	 * pathname.getName().endsWith(".jar"); } });
	 * 
	 * Convert JAR names to URLs URL[] jarUrls = new URL[jarFiles.length]; for
	 * (int i = 0; i < jarFiles.length; i++) { IOUtils.debug().pln("Plugin JAR: "
	 * + jarFiles[i]); try { jarUrls[i] = jarFiles[i].toURI().toURL(); } catch
	 * (MalformedURLException e) { e.printStackTrace(IOUtils.err()); assert false;
	 * } }
	 * 
	 * Inspect the JARs for TheoremProver providers classLoader = new
	 * URLClassLoader(jarUrls, classLoader); }
	 * 
	 * ServiceLoader<TheoremProver.Provider> tpServiceLoader = ServiceLoader.load(
	 * TheoremProver.Provider.class, classLoader);
	 * 
	 * if( Iterables.isEmpty(tpServiceLoader) ) { IOUtils.err().println(
	 * "No theorem prover providers found."); IOUtils.err().println(
	 * "Put plugin JAR in CLASSPATH or " +
	 * DEFAULT_PLUGINS_DIRECTORY.getAbsolutePath()); }
	 * 
	 * Register each TheoremProver provider for (TheoremProver.Provider tp :
	 * tpServiceLoader) { IOUtils.debug().pln("Theorem prover: " + tp.getName());
	 * register(tp); } }
	 */
	...
	public static void discover() {
		...
		// classLoader = new URLClassLoader(urlArr.toArray(new URL[urlArr.size()]),
		// classLoader);
		// }
	...
	/**
	 * Get a <code>TheoremProver</code> instance. If the
	 * <code>OPTION_PROVER</code> preference is set, then an instance matching
	 * that preference will be returned. Otherwise, an arbitrary instance will be
	 * chosen.
	 * 
	 ...
	 *           if no instances are registered, or if an instance satisfying the
	...
	/**
	 * Get a list of options associated with the factory and all of its registered
	 * <code>TheoremProver</code> instances.
	 * 
	 * This method will invoke <code>discover()</code> if it has not previously
	 * been called.
	 */
	...
	/**
	 * Get an SMT-capable <code>TheoremProver</code> instance If multiple
	 * SMT-capable instances are available, an arbitrary instance will be chosen.
	 * 
	 * This method will invoke <code>discover()</code> if it has not previously
	 * been called.
	 * 
	 * @throws TheoremProverFactoryException
	 *           if no SMT-capable instances are available.
	 */
	...
	/**
	 * Get a <code>TheoremProver</code> instance with all of the given
	 * capabilities. If multiple instances have the given capabilities, an
	 * arbitrary instance will be chosen.
	 * 
	 * This method will invoke <code>discover()</code> if it has not previously
	 * been called.
	 * 
	 * @throws TheoremProverFactoryException
	 *           if no instances with all of the given capabilities are available.
	 */
	...
	/**
	 * Return a set of all the registered <code>TheoremProver</code> instances
