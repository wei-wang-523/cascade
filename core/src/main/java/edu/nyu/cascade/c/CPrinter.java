 ...
 *
 * <p />
 * A note on operator precedence: This printer uses precedence levels to control
 * when to print parentheses around expressions. The actual precedence values
 * are the standard C precedence levels multiplied by ten.
 *
 * @author Robert Grimm
...
public class CPrinter extends Visitor {
	...
	/**
	 * The base precedence level. This level corresponds to the expression
	 * nonterminal.
	 */
	public static final int PREC_BASE = 0;

	/**
	 * The list precedence level. This level corresponds to the assignment
	 * expression nonterminal.
	 */
	public static final int PREC_LIST = 11;

	/**
	 * The constant precedence level. This level corresponds to the conditional
	 * expression nonterminal.
	 */
	...
	/**
	 * The flag for whether the current statement requires nesting or for whether
	 * the current declaration is nested within a for statement.
	 */
	protected boolean isNested;

	/**
	 * The flag for whether this statement is the else clause of an if-else
	 * statement.
	 */
	protected boolean isIfElse;

	/**
	 * The flag for whether this compound statement is an expression (as in the
	 * GCC extension).
	 */
	 ...
	 *          The printer.
	 * @param lineUp
	 *          The flag for whether to line up declaratons and statements with
	 ...
	 * definition. A long type definition requires multiple lines for readability.
	 * Examples include enumeration, structure, and union type definitions.
	 *
	 * @param specs
	 *          The generic node, representing a list of specifiers.
	 * @return <code>true</code> if the specifiers contain a long type specifier.
	 */
	...
	/**
	 * Determine whether the specified declaration is long. A long declaration
	 * requires multiple lines for readability. Examples include declarations
	 * containing enumeration, structure, and union type definitions.
	 *
	 * @param decl
	 *          The declaration.
	...
	/**
	 * Print an expression as a truth value. This method prints the specified
	 * node. If that node represents an assignment expression and
	 * {@link #EXTRA_PARENTHESES} is <code>true</code>, this method adds an extra
	 * set of parentheses around the expression to avoid gcc warnings.
	 *
	 * @param n
	 *          The node to print.
	 */
	...
	/**
	 * Start a new statement. This method and the corresponding
	 * {@link #prepareNested()} and {@link #endStatement(boolean)} methods provide
	 * a reasonable default for newlines and indentation when printing statements.
	 * They manage the {@link #isDeclaration}, {@link #isStatement},
	 * {@link #isOpenLine}, {@link #isNested}, and {@link #isIfElse} flags.
	 *
	 * @param kind
	 *          The kind of statement, which must be one of the three statement
	...
	/**
	 * Enter an expression context. The new context is appropriate for an operand
	 * opposite the associativity of the current operator. For example, when
	 * printing an additive expression, this method should be called before
	 * printing the second operand, as additive operators associate left-to-right.
	 *
	 * @see #exitContext(int)
	 *
	 * @return The previous precedence level.
	 ...
	 *
	 * @param prec
	 *          The expression's precedence level.
	 * @return The previous precedence level.
	 */
	...
	public void visitAddressExpression(GNode n) {
		int prec = startExpression(150);
		GNode e = n.getGeneric(0);
		if (e.hasName("AddressExpression") || e.hasName("LabelAddressExpression")) {
			printer.p("&(").p(n.getNode(0)).p(')');
		} else {
			printer.p('&').p(n.getNode(0));
		}
