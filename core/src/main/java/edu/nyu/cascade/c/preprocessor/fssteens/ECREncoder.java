...
class ECREncoder extends Visitor {
  ...
  public ECR visitFunctionCall(GNode node) {
  	...
  	BlankType type = ValueType.blank(
  			size, Parent.getEmpty(), rootScope);
  ...
  public ECR visitPrimaryIdentifier(GNode node) {
    ...
    String scopeKey = peekScopeKey(scope);
  	...
  	if(type.isEnumerator()) return createConstant(NumberT.INT);
  ...
  public ECR visitDirectComponentSelection(GNode node) {
    ...
    return getComponent(addrECR, baseXtcType, fieldName, fieldType);
  }
  ...
  public ECR visitIndirectComponentSelection(GNode node) {
    ...
    Type fieldType = CType.getType(node);
    return getComponent(baseECR, baseXtcType, fieldName, fieldType);
  }
	...
	ECR createForRegion(Expression region, Node ptrNode) {
		...
		Type regionType = CType.getType(ptrNode).resolve().toPointer().getType();
		
		VarImpl regionVar = VarImpl.createRegionVar(name, regionType, scopeName, region);
		uf.add(regionVar);
	...
	private ECR getComponent(ECR srcECR, Type baseXtcType, String fieldName, Type fieldXtcType) {
		...
		case BLANK: {
			...
			assert compatible;
			long offset = cTypeAnalyzer.getOffset(baseXtcType, fieldName);
		...
		default: {
			...
			baseType = uf.getType(base); // refresh base type
			if(!baseType.getParent().equals(base)) {
				// Reset the parent, to the promoted type
			...
			if(fieldXtcType.resolve().isArray()) 
				return srcECR;
			else	
				return base;
		}
