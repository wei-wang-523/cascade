...
public class C32 extends C {
	...
	static {
		GCC_LEGACY_LAYOUT = Limits32.COMPILER_NAME.equals("gcc")
				&& (Limits32.COMPILER_VERSION_MAJOR < 4
						|| Limits32.COMPILER_VERSION_MAJOR == 4);
	}
	...
	public boolean hasThreadLocals() {
		if (null == threadlocal) {
			threadlocal = Limits32.IS_ELF ? Boolean.TRUE : Boolean.FALSE;
		}
	...
	public long getAlignment(Type type, boolean natural) {
		...
		switch (type.tag()) {
		case VOID:
			return Limits32.VOID_ALIGN;

		case BOOLEAN:
			return natural ? Limits32.BOOL_NAT_ALIGN : Limits32.BOOL_ALIGN;
		...
		case FLOAT:
			...
			case SHORT:
			case U_SHORT:
				return natural ? Limits32.SHORT_NAT_ALIGN : Limits32.SHORT_ALIGN;
			case INT:
			case S_INT:
			case U_INT:
				return natural ? Limits32.INT_NAT_ALIGN : Limits32.INT_ALIGN;
			case LONG:
			case U_LONG:
				return natural ? Limits32.LONG_NAT_ALIGN : Limits32.LONG_ALIGN;
			case LONG_LONG:
			case U_LONG_LONG:
				return natural ? Limits32.LONG_LONG_NAT_ALIGN
						: Limits32.LONG_LONG_ALIGN;
			case FLOAT:
				return natural ? Limits32.FLOAT_NAT_ALIGN : Limits32.FLOAT_ALIGN;
			case DOUBLE:
				return natural ? Limits32.DOUBLE_NAT_ALIGN : Limits32.DOUBLE_ALIGN;
			case LONG_DOUBLE:
		...
		case POINTER:
			return natural ? Limits32.POINTER_NAT_ALIGN : Limits32.POINTER_ALIGN;

		case ARRAY:
			return getAlignment(type.resolve().toArray().getType(), natural);

		case FUNCTION:
			return Limits32.FUNCTION_ALIGN;
	...
	public long getSize(Type type) {
		switch (type.tag()) {
		case VOID:
			return Limits32.VOID_SIZE;

		case BOOLEAN:
			return Limits32.BOOL_SIZE;
		...
		case FLOAT:
			...
			case SHORT:
			case U_SHORT:
				return Limits32.SHORT_SIZE;
			case INT:
			case S_INT:
			case U_INT:
				return Limits32.INT_SIZE;
			case LONG:
			case U_LONG:
				return Limits32.LONG_SIZE;
			case LONG_LONG:
			case U_LONG_LONG:
				return Limits32.LONG_LONG_SIZE;
			case FLOAT:
				return Limits32.FLOAT_SIZE;
			case DOUBLE:
				return Limits32.DOUBLE_SIZE;
			case LONG_DOUBLE:
				return Limits32.LONG_DOUBLE_SIZE;
			case FLOAT_COMPLEX:
				return 2 * Limits32.FLOAT_SIZE;
			case DOUBLE_COMPLEX:
				return 2 * Limits32.DOUBLE_SIZE;
			case LONG_DOUBLE_COMPLEX:
			...
			if (!var.hasKind(VariableT.Kind.BITFIELD)) {
				...
				if (isPacked && (GCC_LEGACY_LAYOUT || 0 != width)) {
					...
					if (isLastMember || -1 == members.get(i + 1).getWidth()) {
						size += (bitCount / Limits32.CHAR_BITS);
						if (0 != bitCount % Limits32.CHAR_BITS)
							size++;
					...
					} else if (bitCount + width <= Limits32.toWidth(bitSize)) {
						bitCount += width;
				...
				if (Constants.NAME_GCC.equals(att.getName())) {
					...
					if ("aligned".equals(att.getName())) {
						if (null == att.getValue()) {
							alignment = Math.max(Limits32.LONG_LONG_ALIGN,
									Limits32.LONG_DOUBLE_ALIGN);
						} else {
	...
	public long getWidth(Type number) {
		...
		case INTEGER:
		case FLOAT:
			return getSize(number) * Limits32.CHAR_BITS;
		default:
	...
	public Type fit(BigInteger number) {
		if (Limits32.fitsInt(number)) {
			return NumberT.INT;
		} else if (Limits32.fitsUnsignedInt(number)) {
			return NumberT.U_INT;
		} else if (Limits32.fitsLong(number)) {
			return NumberT.LONG;
		} else if (Limits32.fitsUnsignedLong(number)) {
			return NumberT.U_LONG;
		} else if (Limits32.fitsLongLong(number)) {
			return NumberT.LONG_LONG;
		} else if (Limits32.fitsUnsignedLongLong(number)) {
	...
	public boolean fits(BigInteger number, Type type) {
		switch (type.tag()) {
		case BOOLEAN:
			return Limits32.fitsUnsignedChar(number);
		case INTEGER:
			switch (type.resolve().toInteger().getKind()) {
			case CHAR:
				return Limits32.IS_CHAR_SIGNED ? Limits32.fitsChar(number)
						: Limits32.fitsUnsignedChar(number);
			case S_CHAR:
				return Limits32.fitsChar(number);
			case U_CHAR:
				return Limits32.fitsUnsignedChar(number);
			case SHORT:
				return Limits32.fitsShort(number);
			case U_SHORT:
				return Limits32.fitsUnsignedShort(number);
			case INT:
			case S_INT:
				return Limits32.fitsInt(number);
			case U_INT:
				return Limits32.fitsUnsignedInt(number);
			case LONG:
				return Limits32.fitsLong(number);
			case U_LONG:
				return Limits32.fitsUnsignedLong(number);
			case LONG_LONG:
				return Limits32.fitsLongLong(number);
			case U_LONG_LONG:
				return Limits32.fitsUnsignedLongLong(number);
			}
		default:
	...
	public BigInteger mask(BigInteger number, Type type) {
		...
		case INTEGER:
			switch (type.resolve().toInteger().getKind()) {
			case CHAR:
				return Limits32.IS_CHAR_SIGNED ? Limits32.maskAsSignedChar(number)
						: Limits32.maskAsUnsignedChar(number);
			case S_CHAR:
				return Limits32.maskAsSignedChar(number);
			case U_CHAR:
				return Limits32.maskAsUnsignedChar(number);
			case SHORT:
				return Limits32.maskAsShort(number);
			case U_SHORT:
				return Limits32.maskAsUnsignedShort(number);
			case INT:
			case S_INT:
				return Limits32.maskAsInt(number);
			case U_INT:
				return Limits32.maskAsUnsignedInt(number);
			case LONG:
				return Limits32.maskAsLong(number);
			case U_LONG:
				return Limits32.maskAsUnsignedLong(number);
			case LONG_LONG:
				return Limits32.maskAsLongLong(number);
			case U_LONG_LONG:
				return Limits32.maskAsUnsignedLongLong(number);
			}
		default:
			throw new AssertionError("Not a C integer " + type);
		}
	...
	public Type promote(Type type) {
		...
		case INTEGER:
			...
			case SHORT:
				...
				return NumberT.INT;
			case U_SHORT:
				if (Limits32.SHORT_SIZE < Limits32.INT_SIZE) {
	...
	public Type typeInteger(String literal) {
		...
		if (isUnsigned) {
			if (isLongLong) {
				if (Limits32.fitsUnsignedLongLong(value)) {
					type = NumberT.U_LONG_LONG;
				}

			} else if (isLong) {
				if (Limits32.fitsUnsignedLong(value)) {
					type = NumberT.U_LONG;
				} else if (Limits32.fitsUnsignedLongLong(value)) {
					type = NumberT.U_LONG_LONG;
				}

			} else {
				if (Limits32.fitsUnsignedInt(value)) {
					type = NumberT.U_INT;
				} else if (Limits32.fitsUnsignedLong(value)) {
					type = NumberT.U_LONG;
				} else if (Limits32.fitsUnsignedLongLong(value)) {
					type = NumberT.U_LONG_LONG;
				}
			...
			if (isLongLong) {
				if (Limits32.fitsLongLong(value)) {
					type = NumberT.LONG_LONG;
				}

			} else if (isLong) {
				if (Limits32.fitsLong(value)) {
					type = NumberT.LONG;
				} else if (Limits32.fitsLongLong(value)) {
					type = NumberT.LONG_LONG;
				}

			} else {
				if (Limits32.fitsInt(value)) {
					type = NumberT.INT;
				} else if (Limits32.fitsLong(value)) {
					type = NumberT.LONG;
				} else if (Limits32.fitsLongLong(value)) {
					type = NumberT.LONG_LONG;
				}
			...
			if (isLongLong) {
				...
				if (Limits32.fitsInt(value)) {
					type = NumberT.INT;
				} else if (Limits32.fitsUnsignedInt(value)) {
					type = NumberT.U_INT;
				} else if (Limits32.fitsLong(value)) {
					type = NumberT.LONG;
				} else if (Limits32.fitsUnsignedLong(value)) {
