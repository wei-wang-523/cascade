...
class ECRChecker extends Visitor {
  ...
  private ECRChecker(UnionFindECR uf, SymbolTable symbolTable, ECREncoder ecrEncoder) {
  	this.uf = uf;
  ...
  ECR toLval(Node node) {
	...
	private ECR lookupOpECR(GNode opNode) {
  	...
  	while(scopeItr.hasNext()) {
  		...
  		Triple<GNode, String, String> parentKey = Triple.of(opNode, scopeName, parentScope);
  	...
  	Triple<GNode, String, String> lastKey = Triple.of(opNode, scopeName, lastScope);
