...
public class UnionFindECR {
  	...
  	switch(t1.getKind()) {		
		case BLANK: {
	  	...
	  	String scopeName = CScopeAnalyzer.getTopScope(t1.getScopeName(), t2.getScopeName());
			...
			switch(t2.getKind()) {
			case BLANK:		return ValueType.blank(size, parent, scopeName);
			case SIMPLE:
		...
		case SIMPLE: {
	  	...
	  	String scopeName = CScopeAnalyzer.getTopScope(t1.getScopeName(), t2.getScopeName());
			...
			case SIMPLE: {
				...
				Type xtcType = CType.getBottomType(sim1.getXtcType(), sim2.getXtcType());
		...
		case STRUCT: {
	  	...
	  	String scopeName = CScopeAnalyzer.getTopScope(t1.getScopeName(), t2.getScopeName());
			...
			case STRUCT: {
		  	...
		  	Type xtcType = CType.getBottomType(str1.getXtcType(), str2.getXtcType());
		...
		case OBJECT: {
	  	...
	  	String scopeName = CScopeAnalyzer.getTopScope(t1.getScopeName(), t2.getScopeName());
			...
			case OBJECT: {
				...
				return ValueType.object(ptr1, func1, size, parent, scopeName);
			}
			default:
				throw new IllegalArgumentException(
						"Incompatible kinds " + t1.getKind() + ", " + t2.getKind());			
   ...
   * @return
   */
  boolean makeCompatible(ECR parent, Type type, final String fieldName, Map<Long, ECR> m) {
  	StructOrUnionT suType = type.resolve().toStructOrUnion();
  	long offset = CType.getInstance().getOffset(suType, fieldName);
  	Type fieldType = Iterables.find(suType.getMembers(), new Predicate<VariableT>() {
			@Override
      public boolean apply(VariableT elem) {
        return elem.getName().equals(fieldName);
      }
  	}).getType();
