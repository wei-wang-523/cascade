...
import edu.nyu.cascade.c.preprocessor.steenscfsopt.ValueType.ValueTypeKind;
  	...
  	case STRUCT: {
  		...
  		switch(type2.getKind()) {
			case BOTTOM: {
				...
				setType(e2, ValueType.struct(fieldMapCopy, rangeSize, Parent.getBottom(), type2.hasOpTag()));
				return; 
  		}
			case BLANK: {
				RangeMap<Long, ECR> fieldMapCopy = FieldRangeMap.create();
				fieldMapCopy.putAll(type1.asStruct().getFieldMap());
				Size size2 = type2.getSize();
				setType(e2, ValueType.struct(fieldMapCopy, size2, type2.getParent(), type2.hasOpTag()));
				if(!Size.isLessThan(rangeSize, size2)) expand(e2, rangeSize);
				return;
			}
			case SIMPLE: {
...
//				return;
  ...
  private void addField(StructType structT, ECR fieldAddrECR, Range<Long> fieldInterval) {
	 ...
	 * Collapse <code>structECR</code> by merge it 
	 * with all its element ECRs, and set its type
	 * as the cell type (simple type might with top size)
	 * 
	 * @param structECR
	 * @param structT
