...
/**
 * The global state for parsing C. This class provides a simplified symbol table
 * that is organized as a stack of parsing contexts, with a new context being
 * pushed onto the stack through {@link #start()} and popped again through
...
public class CParserState implements State {
	...
	protected static class Context {
		...
		/**
		 * The flags for this context. The flags from LSB to MSB are: FLAG_TYPEDEF,
		 * FLAG_SCOPE, FLAG_TYPE_SPEC, FLAG_PARAMS, FLAG_MODIFIED, FLAG_STRUCTURE.
		 */
		public int flags;
	...
	/**
	 * The top of the context stack. The implementation assumes that this field
	 * always references at least one context, which corresponds to the global
	 * namespace.
	 */
	...
	public void functionDeclarator() {
		if (DEBUG)
			System.out.println("functionDeclarator()");
	...
	public void pushScope() {
		if (DEBUG)
			System.out.println("pushScope(" + nesting + ")");
		top.set(FLAG_SCOPE);
	}
	...
	public void popScope() {
		if (DEBUG)
			System.out.println("popScope(" + nesting + ")");
	...
	public void enterStructure() {
		if (DEBUG)
			System.out.println("enterStructure(" + nesting + ")");
	...
	public void exitStructure() {
		if (DEBUG)
			System.out.println("exitStructure(" + nesting + ")");
		top.clear(FLAG_STRUCTURE);
	}

	/**
	 * Implicitly bind the specified identifier. Depending on the current parsing
	 ...
	 *          The identifier.
	 * @param isType
	 *          The flag for whether the identifier represents a type.
	 */
	...
	/**
	 * Determine whether a declaration actually is a declaration. This method
	 ...
	 * </pre>
	 * 
	 * can actually represent a declaration. It assumes that any type specifier
	 * encountered while parsing <code>DeclarationSpecifiers</code> has been
	 * marked through {@link #typeSpecifier()}.
	 *
	 * @param idl
	 *          The result of parsing the optional initialized declarator list.
	...
	/**
	 * Record a line marker. Note that string values for the four flags are
	 * interpreted as follows: Any non-null string counts for <code>true</code>,
	 * while null counts for <code>false</code>.
	 *
	 * @see LineMarker
	 *
	 * @param file
	 *          The file name (without quotes).
	 * @param line
	 *          The line number.
	 * @param isStartFile
	 *          The start file flag.
	 * @param isReturnToFile
	...
	public void lineMarker(String file, int line, String isStartFile,
		...
		int flags = 0;
		if (null != isStartFile)
			flags |= LineMarker.FLAG_START_FILE;
		if (null != isReturnToFile)
			flags |= LineMarker.FLAG_RETURN_TO_FILE;
		if (null != isSystemHeader)
			flags |= LineMarker.FLAG_SYSTEM_HEADER;
		if (null != isExternC)
			flags |= LineMarker.FLAG_EXTERN_C;
	 ...
	 * @param location
	 *          The pragma's source location.
	 */
	public void pragma(String directive, Location location) {
		if (DEBUG)
			System.out.println("pragma(" + directive + ")");
	 ...
	 *
	 * @param ident
	 *          The actual identity marker.
	 * @param location
	 *          The ident directive's source location.
	 */
	...
	/**
	 * Mark the current annotation. This method must be called before recognizing
	 * the nonterminals to be annotated. Furthermore, it must be called within the
	 * context of a stateful production.
	 */
	public void mark() {
		if (DEBUG)
	...
	/**
	 * Annotate the specified node. If any annotations have been recorded and
	 * {@link #mark() marked}, the specified node is wrapped by those annotations
	 * and the outer-most annotation is returned. Otherwise, the specified node is
	 * returned.
	 *
	 * @param node
	 *          The node.
	...
	public Node annotate(Node node) {
		if (DEBUG)
			System.out.println("annotate(" + node + ")");
