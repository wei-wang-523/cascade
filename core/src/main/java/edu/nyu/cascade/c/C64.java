...
public class C64 extends C {
	...
	static {
		GCC_LEGACY_LAYOUT = Limits64.COMPILER_NAME.equals("gcc")
				&& (Limits64.COMPILER_VERSION_MAJOR < 4
						|| Limits64.COMPILER_VERSION_MAJOR == 4);
	}
	...
	/**
	 * The canonical implicit int type. In K&R C, a missing type specifier is
	 * treated as an int. We preserve knowledge about this lack of type specifier
	 * through this type, which has a {@link Constants#ATT_IMPLICIT} attribute.
	 */
	...
	protected static final NumberT.Kind KIND_SIZEOF = IntegerT
			.fromRank(Limits64.SIZEOF_RANK, false);
	...
	protected static final NumberT.Kind KIND_PTR_DIFF = IntegerT
			.fromRank(Limits64.PTRDIFF_RANK, true);
	...
	protected static final NumberT.Kind KIND_WCHAR = IntegerT
			.fromRank(Limits64.WCHAR_RANK, Limits64.IS_WCHAR_SIGNED);
	 ...
	 *
	 * <p />
	 * Per C99 6.2.5, a type is incomplete if it does not contain sufficient
	 * information for determining its size. However, per C99 6.7.2.1, a struct
	 * type with more than one named member may have an incomplete type as its
	 * last member, while still being considered complete.
	 *
	 * @param type
	 *          The type.
	...
	/**
	 * Determine whether the specified type has a trailing array. This method
	 * checks whether the specified type is either a struct type with an
	 * incomplete array as its last member or a union containing such a member.
	 *
	 * @param type
	 *          The type.
	...
	/**
	 * Determine whether the specified type is variably modified. Consistent with
	 * C99 6.7.5-3, this method checks whether the type contains a variable length
	 * array type.
	 *
	 * @param type
	 *          The type.
	...
	/**
	 * Determine whether the specified type is qualified as constant. Consistent
	 * with C99 6.3.2.1, this method checks whether the type has a
	 * {@link Constants#ATT_CONSTANT} attribute and, if it is an array, struct, or
	 * union, whether any member or element of the type has that attribute.
	 *
	 * @param type
	 *          The type.
	 * @return <code>true</code> if the specified type is constant qualified.
	 */
	 ...
	 * Consistent with C99 6.3.2.1, this method checks that the type represents an
	 * lvalue, is not incomplete, and is not qualified as constant. For struct and
	 * union types, this method also checks that no member is qualified as
	 * constant.
	 *
	 * @param type
	 *          The type.
	 * @return <code>true</code> if the type represents a modifiable lvalue.
	 */
	...
	/**
	 * Determine whether this type has a constant reference. This method takes the
	 * pointer decay of arrays and functions into account and returns
	 * <code>true</code> either if the specified type has a constant reference or
	 * if this type is an array or function and has a constant reference.
	 *
	 * @param type
	 *          The type.
	 ...
	 *           Signals that the type does not have a constant reference.
	 */
	...
	/**
	 * Determine whether the specified type has at least the qualifiers of the
	 * specified template.
	 *
	 * @param type
	 *          The type.
	...
	/**
	 * Determine whether the specified type has the same qualifiers as the
	 * specified template.
	 *
	 * @param type
	 *          The type.
	 * @param template
	 *          The template.
	 * @return <code>true</code> if the type has the same qualifiers as the
	 *         template.
	 */
	 ...
	 * If the template has any qualifiers, this method first
	...
	/**
	 * Convert the specified type to an rvalue. If the specified type has a shape,
	 * this method returns the resolved type, annotated with any qualifiers and
	 * constant value. Otherwise, it returns the specified type.
	 *
	 * @param type
	 *          The type.
	 ...
	 *
	 * @return <code>true</code> if the current target supports thread-local
	 *         storage.
	 */
	public boolean hasThreadLocals() {
		if (null == threadlocal) {
			threadlocal = Limits64.IS_ELF ? Boolean.TRUE : Boolean.FALSE;
		}
	...
	/**
	 * Get the specified type's alignment in bytes. A type's natural alignment is
	 * the type's alignment outside arrays, structures, and unions.
	 *
	 * @param type
	 *          The type.
	 * @param natural
	 *          The flag for determining the natural alignment.
	 * @return The type's alignment.
	 * @throws IllegalArgumentException
	 *           Signals that the type does not have a static alignment.
	 */
	public long getAlignment(Type type, boolean natural) {
		...
		switch (type.tag()) {
		case VOID:
			return Limits64.VOID_ALIGN;

		case BOOLEAN:
			return natural ? Limits64.BOOL_NAT_ALIGN : Limits64.BOOL_ALIGN;
		...
		case FLOAT:
			...
			case SHORT:
			case U_SHORT:
				return natural ? Limits64.SHORT_NAT_ALIGN : Limits64.SHORT_ALIGN;
			case INT:
			case S_INT:
			case U_INT:
				return natural ? Limits64.INT_NAT_ALIGN : Limits64.INT_ALIGN;
			case LONG:
			case U_LONG:
				return natural ? Limits64.LONG_NAT_ALIGN : Limits64.LONG_ALIGN;
			case LONG_LONG:
			case U_LONG_LONG:
				return natural ? Limits64.LONG_LONG_NAT_ALIGN
						: Limits64.LONG_LONG_ALIGN;
			case FLOAT:
				return natural ? Limits64.FLOAT_NAT_ALIGN : Limits64.FLOAT_ALIGN;
			case DOUBLE:
				return natural ? Limits64.DOUBLE_NAT_ALIGN : Limits64.DOUBLE_ALIGN;
			case LONG_DOUBLE:
				return natural ? Limits64.LONG_DOUBLE_NAT_ALIGN
						: Limits64.LONG_DOUBLE_ALIGN;
			case FLOAT_COMPLEX:
				return natural ? Limits64.FLOAT_NAT_ALIGN : Limits64.FLOAT_ALIGN;
			case DOUBLE_COMPLEX:
				return natural ? Limits64.DOUBLE_NAT_ALIGN : Limits64.DOUBLE_ALIGN;
		...
		case POINTER:
			return natural ? Limits64.POINTER_NAT_ALIGN : Limits64.POINTER_ALIGN;

		case ARRAY:
			return getAlignment(type.resolve().toArray().getType(), natural);

		case FUNCTION:
			return Limits64.FUNCTION_ALIGN;
	 ...
	 * @throws IllegalArgumentException
	 *           Signals that the type does not have a static size.
	 */
	public long getSize(Type type) {
		switch (type.tag()) {
		case VOID:
			return Limits64.VOID_SIZE;

		case BOOLEAN:
			return Limits64.BOOL_SIZE;
		...
		case FLOAT:
			...
			case SHORT:
			case U_SHORT:
				return Limits64.SHORT_SIZE;
			case INT:
			case S_INT:
			case U_INT:
				return Limits64.INT_SIZE;
			case LONG:
			case U_LONG:
				return Limits64.LONG_SIZE;
			case LONG_LONG:
			case U_LONG_LONG:
				return Limits64.LONG_LONG_SIZE;
			case FLOAT:
				return Limits64.FLOAT_SIZE;
			case DOUBLE:
				return Limits64.DOUBLE_SIZE;
			case LONG_DOUBLE:
				return Limits64.LONG_DOUBLE_SIZE;
			case FLOAT_COMPLEX:
				return 2 * Limits64.FLOAT_SIZE;
			case DOUBLE_COMPLEX:
				return 2 * Limits64.DOUBLE_SIZE;
			case LONG_DOUBLE_COMPLEX:
				return 2 * Limits64.LONG_DOUBLE_SIZE;
			default:
				throw new AssertionError(
						"Invalid number kind " + type.toNumber().getKind());
			}
		...
		case ARRAY: {
			...
			if (array.hasLength()) {
				...
				IOUtils.errPrinter().pln("Array without size");
				return 0;
			}
		}
	 ...
	 * @param name
	 *          The member's name.
	 * @return The offset or -1 if the structure or union does not have a member
	 *         with the specified name or the member is a bit-field.
	 */
	 ...
	 *          The structure.
	 * @param name
	 *          The member name or <code>null</code> for the entire structure.
	 * @return The offset/size or -1 if the structure does not have a member with
	 *         the specified name or the member is a bit-field.
	 */
	public long layout(StructT type, String name) {
		...
		long size = 0;
		long bitCount = 0; // The number of bits we have seen so far.
		long bitSize = 0; // The size of the member we are filling with bits.
		long bitAlign = 1; // The alignment of the member we are filling with bits.
		...
		for (int i = 0; i < memberCount; i++) {
			...
			if (!var.hasKind(VariableT.Kind.BITFIELD)) {
				...
				if (isPacked && (GCC_LEGACY_LAYOUT || 0 != width)) {
					// In packed structures, a bit-field just adds the specified
					// number of bits to the structure. GCC 4.0 and earlier
					// ignore the member's alignment for zero width bit-fields.
					bitCount += width;

					if (isLastMember || -1 == members.get(i + 1).getWidth()) {
						size += (bitCount / Limits64.CHAR_BITS);
						if (0 != bitCount % Limits64.CHAR_BITS)
							size++;
					...
					} else if (bitCount + width <= Limits64.toWidth(bitSize)) {
						bitCount += width;
	...
	private static final Attribute PACKED = new Attribute(Constants.NAME_GCC,
			new Attribute("packed", null));
	...
	/**
	 * Get the specified type's alignment. If the specified type has a GCC aligned
	 * attribute, this method returns the corresponding alignment. Otherwise, it
	 * returns -1.
	 *
	 * @param type
	 *          The type.
	 * @return The alignment or -1 if the type does not have an aligned attribute.
	 */
				...
				if (Constants.NAME_GCC.equals(att.getName())) {
					...
					if ("aligned".equals(att.getName())) {
						if (null == att.getValue()) {
							alignment = Math.max(Limits64.LONG_LONG_ALIGN,
									Limits64.LONG_DOUBLE_ALIGN);
						} else {
	...
	public long getWidth(Type number) {
		...
		case INTEGER:
		case FLOAT:
			return getSize(number) * Limits64.CHAR_BITS;
		default:
	...
	/**
	 * Fit the specified number to the closest integer type, starting with
	 * <code>int</code>.
	 *
	 * @param number
	 *          The number.
	 * @return The closest containing integer type or {@link ErrorT#TYPE} if the
	 *         number is too large.
	 */
	public Type fit(BigInteger number) {
		if (Limits64.fitsInt(number)) {
			return NumberT.INT;
		} else if (Limits64.fitsUnsignedInt(number)) {
			return NumberT.U_INT;
		} else if (Limits64.fitsLong(number)) {
			return NumberT.LONG;
		} else if (Limits64.fitsUnsignedLong(number)) {
			return NumberT.U_LONG;
		} else if (Limits64.fitsLongLong(number)) {
			return NumberT.LONG_LONG;
		} else if (Limits64.fitsUnsignedLongLong(number)) {
	...
	public boolean fits(BigInteger number, Type type) {
		switch (type.tag()) {
		case BOOLEAN:
			return Limits64.fitsUnsignedChar(number);
		case INTEGER:
			switch (type.resolve().toInteger().getKind()) {
			case CHAR:
				return Limits64.IS_CHAR_SIGNED ? Limits64.fitsChar(number)
						: Limits64.fitsUnsignedChar(number);
			case S_CHAR:
				return Limits64.fitsChar(number);
			case U_CHAR:
				return Limits64.fitsUnsignedChar(number);
			case SHORT:
				return Limits64.fitsShort(number);
			case U_SHORT:
				return Limits64.fitsUnsignedShort(number);
			case INT:
			case S_INT:
				return Limits64.fitsInt(number);
			case U_INT:
				return Limits64.fitsUnsignedInt(number);
			case LONG:
				return Limits64.fitsLong(number);
			case U_LONG:
				return Limits64.fitsUnsignedLong(number);
			case LONG_LONG:
				return Limits64.fitsLongLong(number);
			case U_LONG_LONG:
				return Limits64.fitsUnsignedLongLong(number);
			}
		default:
	...
	public BigInteger mask(BigInteger number, Type type) {
		...
		case INTEGER:
			switch (type.resolve().toInteger().getKind()) {
			case CHAR:
				return Limits64.IS_CHAR_SIGNED ? Limits64.maskAsSignedChar(number)
						: Limits64.maskAsUnsignedChar(number);
			case S_CHAR:
				return Limits64.maskAsSignedChar(number);
			case U_CHAR:
				return Limits64.maskAsUnsignedChar(number);
			case SHORT:
				return Limits64.maskAsShort(number);
			case U_SHORT:
				return Limits64.maskAsUnsignedShort(number);
			case INT:
			case S_INT:
				return Limits64.maskAsInt(number);
			case U_INT:
				return Limits64.maskAsUnsignedInt(number);
			case LONG:
				return Limits64.maskAsLong(number);
			case U_LONG:
				return Limits64.maskAsUnsignedLong(number);
			case LONG_LONG:
				return Limits64.maskAsLongLong(number);
			case U_LONG_LONG:
				return Limits64.maskAsUnsignedLongLong(number);
			}
		default:
			throw new AssertionError("Not a C integer " + type);
		}
	...
	/**
	 * Integer promote the specified type. This method resolves the type and, if
	 * the type is integral, then performs C's integer promotion (C99 6.3.1.1).
	 * Additionally, it normalizes implicit and signed int types to int types.
	 *
	 * @param type
	 *          The type.
	...
	public Type promote(Type type) {
		...
		// signed.
		boolean flip = ((!Limits64.IS_INT_SIGNED) && type.hasVariable()
		...
		case INTEGER:
			...
			case SHORT:
				...
				return NumberT.INT;
			case U_SHORT:
				if (Limits64.SHORT_SIZE < Limits64.INT_SIZE) {
	...
	/**
	 * Argument promote this type. This method resolves the type and, if the type
	 * is an integral type or a float, then performs C's default argument
	 * promotions (C99 6.5.2.2).
	 *
	 * @param type
	 *          The type.
	...
	/**
	 * Pointerize the specified type. This method resolves the type and, if the
	 * type is an array or function, then performs C's pointer decay (C99
	 * 6.3.2.1).
	 *
	 * @param type
	 *          The type.
	...
	/**
	 * Perform the usual arithmetic conversions. Per C99 6.3.1.8, this method
	 * performs the usual arithmetic conversions for the specified two types and
	 * returns the type of the corresponding result.
	 *
	 * @param t1
	 *          The first type.
	 * @param t2
	 ...
	 * @throws IllegalArgumentException
	 *           Signals that either type is not arithmetic.
	 */
	public Type convert(Type t1, Type t2) {
		...
		} else if ((NumberT.Kind.DOUBLE == k1) || (NumberT.Kind.DOUBLE == k2)) {
			return NumberT.DOUBLE;

		} else if ((NumberT.Kind.FLOAT == k1) || (NumberT.Kind.FLOAT == k2)) {
	...
	/**
	 * Compose the specified types. This method determines whether the two types
	 * are compatible while also constructing a composite type as specified in C99
	 * 6.2.7. If the types are compatible, the resulting type is wrapped exactly
	 * as the first type. If the types are not compatible, the resulting type is
	 * {@link ErrorT#TYPE}.
	 *
	 * <p />
	 * Note that if both types are derived types, this method ensures that any
	 * referenced types have the same qualifiers. However, it does not ensure that
	 * the two types have the same qualifiers. As a result, two types
	 * <code>t1</code> and <code>t2</code> are compatible if:
	 * 
	 * <pre>
	 ...
	 *
	 * <p />
	 * Further note that the composed type does not preserve any annotations or
	 * wraped types and thus needs to be annotated with the two type's qualifiers
	 * etc.
	 *
	 * @see #equal(Type,Type)
	 *
	 ...
	 *          The second type.
	 * @param pedantic
	 *          The flag for pedantic composition.
	 ...
	 *          The flag for pedantic composition.
	 * @param recursive
	 *          The flag for recursive invocations.
	 * @return The composed type.
	 */
	protected Type compose(Type t1, Type t2, boolean pedantic,
			boolean recursive) {
		if (recursive) {
		...
		case INTERNAL:
			return t1.toInternal().getName().equals(t2.toInternal().getName()) ? t1
					: ErrorT.TYPE;

		case LABEL:
			return t1.toLabel().getName().equals(t2.toLabel().getName()) ? t1
					: ErrorT.TYPE;
	...
	protected Type composeArrays(ArrayT a1, ArrayT a2) {
		if (!hasSameQualifiers(a1.getType(), a2.getType()))
			return ErrorT.TYPE;
		...
		return el == a1.getType() ? a1
				: new ArrayT(a1, el, a1.isVarLength(), a1.getLength());
	}

	/**
	 * Compose the specified function types (C99 6.2.7). Note that this method
	 * ignores any exceptions, which are not part of the C language anyway.
	 *
	 * @param f1
	 *          The first function.
	 * @param f2
	 ...
	 * @return The composed type.
	 */
	protected Type composeFunctions(FunctionT f1, FunctionT f2,
			boolean pedantic) {
		// Compare the names.
		if (null == f1.getName()) {
			if (null != f2.getName())
		...
		// Compare the results.
		if (!hasSameQualifiers(f1.getResult(), f2.getResult()))
			return ErrorT.TYPE;
		...
		for (int i = 0; i < size; i++) {
			...
			if (pedantic && !hasSameQualifiers(p1, p2))
				return ErrorT.TYPE;
	...
	/**
	 * Determine whether the specified types are equal to each other. Calling this
	 * method on types <code>t1</code> and <code>t2</code> is equivalent to:
	 * 
	 * <pre>
	...
	protected final BigInteger FACTOR_CHAR = BigInteger.valueOf(2)
			.pow(Limits64.CHAR_BITS);

	/** The factor for wide chars. */
	protected final BigInteger FACTOR_WIDE = BigInteger.valueOf(2)
			.pow(Limits64.CHAR_BITS * Limits64.WCHAR_SIZE);

	/**
	 * Type the specified C character literal. This method determines the type for
	 * the specified character literal, which may be a wide character literal, and
	 * returns that type. The type is annotated with the literal's constant value,
	 * even if the value does not fit the type.
	 *
	 * @param literal
	 *          The literal.
	...
	/**
	 * Type the specified integer literal. This method returns the type for the
	 * specified C integer literal wrapped in the literal's constant value. If the
	 * specified literal does not fit any type, this method returns the largest
	 * appropriate type.
	 *
	 * @param literal
	 *          The literal.
	...
	public Type typeInteger(String literal) {
		...
		if (isUnsigned) {
			if (isLongLong) {
				if (Limits64.fitsUnsignedLongLong(value)) {
					type = NumberT.U_LONG_LONG;
				}

			} else if (isLong) {
				if (Limits64.fitsUnsignedLong(value)) {
					type = NumberT.U_LONG;
				} else if (Limits64.fitsUnsignedLongLong(value)) {
					type = NumberT.U_LONG_LONG;
				}

			} else {
				if (Limits64.fitsUnsignedInt(value)) {
					type = NumberT.U_INT;
				} else if (Limits64.fitsUnsignedLong(value)) {
					type = NumberT.U_LONG;
				} else if (Limits64.fitsUnsignedLongLong(value)) {
					type = NumberT.U_LONG_LONG;
				}
			...
			if (isLongLong) {
				if (Limits64.fitsLongLong(value)) {
					type = NumberT.LONG_LONG;
				}

			} else if (isLong) {
				if (Limits64.fitsLong(value)) {
					type = NumberT.LONG;
				} else if (Limits64.fitsLongLong(value)) {
					type = NumberT.LONG_LONG;
				}

			} else {
				if (Limits64.fitsInt(value)) {
					type = NumberT.INT;
				} else if (Limits64.fitsLong(value)) {
					type = NumberT.LONG;
				} else if (Limits64.fitsLongLong(value)) {
					type = NumberT.LONG_LONG;
				}
			...
			if (isLongLong) {
				...
				if (Limits64.fitsInt(value)) {
					type = NumberT.INT;
				} else if (Limits64.fitsUnsignedInt(value)) {
					type = NumberT.U_INT;
				} else if (Limits64.fitsLong(value)) {
					type = NumberT.LONG;
				} else if (Limits64.fitsUnsignedLong(value)) {
	...
	/**
	 * Type the specified floating point literal. This method returns the type for
	 * the specified C floating point literal wrapped in the literal's constant
	 * value.
	 *
	 * @param literal
	 *          The literal.
