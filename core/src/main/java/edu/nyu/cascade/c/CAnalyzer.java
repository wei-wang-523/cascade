...
public class CAnalyzer extends Visitor {
	...
	public class Specifiers extends Visitor {
		...
		/**
		 * Create a new sequence of specifiers. This constructor analyzes the
		 * specified AST node and may print error messages as a result.
		 *
		 * @param specifiers
		 *          The node holding the declaration specifiers (which may be
		 *          <code>null</code>).
		 * @param refIsDecl
		 *          The flag for whether a struct/union reference also is a
		...
		/**
		 * Get the base type. The base type has been annotated with all attributes
		 * besides the storage class and function specifier attributes.
		 *
		 ...
		 * @param att
		 *          The attribute.
		 * @return <code>true</code> if the specifiers contain the specified
		 *         attribute.
		 */
		public boolean contains(Attribute att) {
			if ((null != attributes) && attributes.contains(att))
				return true;
			if (att.equals(storage))
				return true;
			if (att.equals(threadlocal))
				return true;
			return att.equals(function);
		}

		/**
		 * Determine whether the specifiers include any attributes besides a
		 * function specifier, storage class, and thread-local specifier.
		 *
		 * @return <code>true</code> if the specifiers include any attributes
		 *         besides a function specifier, storage class, and thread-local
		 *         specifier.
		 */
		 ...
		 *
		 * @return <code>true</code> if the specifiers include a function specifier.
		 */
		...
		/**
		 * Determine whether the specifiers include a thread-local specifier.
		 *
		 * @return <code>true</code> if the specifiers include a thread-local
		 *         specifier.
		 */
		 ...
		 *
		 * @return The storage class or <code>null</code> if the specifiers do
		 *         contain one.
		 */
		...
		/**
		 * Annotate the specified base type. This method annotates the specified
		 * type with all attributes besides the storage class, thread-local
		 * specifier, and function specifier attributes.
		 *
		 * @param base
		 *          The base type.
		...
		/**
		 * Annotate the specified full type. This method annotates the specified
		 * type with any storage class, thread-local specifier, and function
		...
		public Type annotateFull(Type full) {
			...
			if (null != storage || null != threadlocal || null != function) {
				if (type == full)
					full = full.annotate();
				if (null != storage)
					full = full.attribute(storage);
				if (null != threadlocal)
					full = full.attribute(threadlocal);
				if (null != function)
					full = full.attribute(function);
			}
			return full;
		}
		...
		/**
		 * Check that the tag declaration is not located within a parameter list. If
		 * the declaration is located within a parameter list, this method prints
		...
		/**
		 * Process the structure declaration list. Note that this method assumes
		 * that the current symbol table scope is the corresponding struct's or
		 * union's scope.
		 */
					...
					while (iter.hasNext()) {
						...
						if (isBitField) {
							...
							if ((!resolved.isBoolean())
									&& ((!resolved.isInteger()) || (resolved.isInteger()
											&& pedantic && (!NumberT.equal(NumberT.Kind.INT, kind))
							...
							if (widthType.hasError()) {
								...
								if (null == identifier) {
									...
									runtime.error(
											"bit-field '" + name + "' width not an integer constant",
											widthExpr);
								}
								// Patch in width.
								width = 0;
								...
								try {
									...
									if (null == identifier) {
										...
										runtime.error(
												"can't compute width in bit-field '" + name + "'",
												widthExpr);
									}
								...
								if (widthValue.compareTo(BigInteger.valueOf(typeWidth)) > 0) {
									if (null == identifier) {
										...
										runtime.error(
												"bit-field '" + name + "' width exceeds its type",
												widthExpr);
									}
									// Patch in width.
									width = typeWidth;
							...
							if (!checkType(declaration, name, type)) {
								...
								if (c().isIncomplete(array.getType())
									...
									if (!isStruct) {
										runtime.error(
												"flexible array member '" + name + "' in union",
												declaration);
								...
								if (pedantic) {
									runtime.error(
											"field '" + name + "' has variably modified " + "type",
											declaration);
								} else if (isTopLevel
										&& (!TMP_SCOPE.equals(table.current().getName()))) {
		...
		public void visitEnumerationTypeDefinition(GNode n) {
			if (hasType()) {
				...
				final List<EnumeratorT> types = new ArrayList<EnumeratorT>(
						enumerators.size());
				...
				final Type baseT;
				if (cops.fitsInt(min) && cops.fitsInt(max)) {
					baseT = NumberT.INT;
				} else if (cops.fitsUnsignedInt(min) && cops.fitsUnsignedInt(max)) {
					baseT = NumberT.U_INT;
				} else if (cops.fitsLong(min) && cops.fitsLong(max)) {
					baseT = NumberT.LONG;
				} else if (cops.fitsUnsignedLong(min) && cops.fitsUnsignedLong(max)) {
	...
	public class Initializer {
		 ...
		 *          The type.
		 * @param auto
		 *          The flag for automatic storage.
		 */
			...
			for (int cursor = 0; cursor < num; cursor++) {
				...
				if (initializer.hasName("InitializerList")) {
					...
					case POINTER: {
						...
						break;
					case ARRAY: {
						new Initializer(initializer, element,
								element.toArray().getType().resolve(), auto).process();
					}
						break;
					default:
						...
						if (isInitializable(element, right)) {
							...
							if (1 <= DEBUG) {
								...
								if (right.hasConstant()) {
									runtime.console()
											.p(right.getConstant().getValue().toString());
			...
			if (null == designation) {
				while (true) {
					...
					if ((-1 == size) || (index < size)) {
						...
						} else if ((!hasSize(type)) && (0 == states.size()) && top) {
		...
		private void pop() {
			// DEBUG: Pop.
			if (2 <= DEBUG)
				runtime.console().indent().pln("pop").flush();
		...
		/**
		 * Get the result type. If this initializer list is not nested and has an
		 * incomplete array as its type, this method patches the array's size.
		 * Otherwise, it returns the overall type.
		 *
	...
	/**
	 * The name of the node property indicating that a node has already been
	 * marked with its type during initializer processing.
	 */
	...
	/**
	 * The flag for whether the current declaration is top-level, that is, an
	...
	/**
	 * The list of loop statement flags. For each nested loop statement, this list
	 * contains exactly one boolean.
	 */
	protected List<Boolean> loops;

	/**
	 * The list of switch statement flags. For each nested switch statement, this
	 * list contains exactly one boolean, which is <code>true</code> if a default
	 * label has been seen.
	 */
	protected List<Boolean> switches;

	/**
	 * The flag for a statement as expression node. This flag is <code>true</code>
	 * if the current compound statement is nested within a statement as
	 * expression node.
	 */
	protected boolean isStmtAsExpr;

	/**
	 * The list of completeness checks to perform at the end of a translation
	 * unit.
	 */
	protected List<CompletenessCheck> checks;

	/**
	 * Create a new C analyzer. The newly created analyzer uses {@link C} for
			...
			for (Object o : initDeclList) {
				...
				if (spec.contains(Constants.ATT_STORAGE_TYPEDEF)) {
					...
					if (table.current().isDefinedLocally(name)) {
						...
						table.current().define(name,
								new AliasT(name, type).locate(n).seal());
					}
					...
					if (resolved.isFunction()) {
						...
						if (null != initializer) {
							runtime.error(
									"function '" + name + "' is initialized like a " + "variable",
									initDecl);
						}
					...
					if (isTopLevel) {
						...
						if (!resolved.isFunction()) {
							if (type.hasAttribute(Constants.ATT_STORAGE_AUTO)) {
								runtime.error(
										"file-scope declaration of '" + name + "' specifies 'auto'",
								...
								if (pedantic || null == initDecl.get(2)) {
									runtime.error(
											"file-scope declaration of '" + name
													+ "' specifies 'register'",
						...
						if (null != extern || global) {
							final Type previous = global
									? (Type) table.current().lookupLocally(name) : extern;
							...
							if (resolved.isFunction()) {
								if (type.hasAttribute(Constants.ATT_STORAGE_AUTO)
									...
									type.removeAttribute(
											type.getAttribute(Constants.NAME_STORAGE));
								}
					...
					if ((null != initializer) && (!resolved.isFunction())) {
						if (type.hasAttribute(Constants.ATT_STORAGE_EXTERN)) {
							if (isTopLevel) {
								...
								runtime.error(
										"'" + name + "' has both 'extern' and initializer",
										initDecl);
							}
						} else {
							type = type.attribute(Constants.ATT_DEFINED);
	...
	/**
	 * Process the specified initializer. This method checks that the specified
	 * initializer is consistent with the specified type. It also completes the
	 * type based on the literal.
	 *
	 * @param expr
	 *          The overall expression node.
	 * @param name
	 *          The name of the variable being initialized, which may be
	 *          <code>null</code>.
	 * @param left
	 *          The left-hand type.
	 * @param literal
	...
	/**
	 * Process the string sizes for the specified types. If the specified types
	 * are not compatible string types or the right-hand type does no represent a
	 * string constant, this method simply returns the specified left-hand type.
	 * Otherwise, if the left-hand type is an incomplete array and the expression
	 * is not nested, this method updates a copy of the left-hand type and returns
	 * that copy. If the left-hand type is a complete array and the right-hand
	 * type is longer, it emits a warning.
	 *
	 * @param expr
	 *          The overall expression node.
	 * @param desc
	 *          The description.
	 * @param nested
	 *          The flag for nested initializers.
	 * @param left
	 *          The left-hand type.
	 * @param right
	 *          The right-hand type.
	...
	/**
	 * Create <code>__func__</code>'s type for the function with the specified
	 * name.
	 *
	 * @param name
	 *          The function name.
	...
	/**
	 * Process the specified function's parameters. This method checks that all
	 * parameters are named and have valid and complete types. Furthermore, for
	 * old-style function definitions, it checks that the parameter types are
	 * consistent with any previous new-style declaration and, for previous
	 * old-style declarations, updates the function's type with the parameter
	 * types. Finally, it adds the appropriate definitions to the current symbol
	 * table scope.
	 *
	 * <p />
	 * Note that the specified type is expected to be the result of composing the
	 * defined type with any previous declarations. For old-style definitions,
	 * this type contains the parameter types of any previous new-style
	 * declaration or is old-style as well, if any previous declarations are also
	 * old-style.
	 *
	 * @param node
	 *          The node representing the function.
	 * @param function
	 *          The function's type.
	 */
				...
				for (Object o : declarations) {
					...
					if (null == declaration.get(2)) {
						...
						for (Object o2 : declaration.getGeneric(2)) {
							...
							final GNode identifier = getDeclaredId(declarator);
							final String name = identifier.getString(0);
							Type type = getDeclaredType(true, spec.getBaseType(), declarator);
							...
							case ARRAY:
								type = c().qualify(
										new PointerT(type.resolve().toArray().getType()), type);
								break;
							case FUNCTION:
							...
							if ((type.hasAttribute(Constants.NAME_STORAGE)
									...
									|| type.hasAttribute(Constants.ATT_THREAD_LOCAL)) {
								runtime.error(
										"storage class specified for parameter '" + name + "'",
										declaration);
							} else if (!type.hasAttribute(Constants.NAME_STORAGE)) {
								type = type.attribute(Constants.ATT_STORAGE_AUTO);
							}
					...
					for (int i = 0; i < size; i++) {
						...
						if (t1.hasError() || t2.hasError()) {
							...
							runtime.error("type qualifiers of argument '"
									+ t1.toVariable().getName() + "' don't match prototype",
									node);
						}
					}
				}
	...
	/**
	 * Check that all labels used in the specified top-level function are also
	 * defined. This method requires that all relevant AST nodes have been
	 * associated with their symbol table {@link Constants#SCOPE scopes}.
	 *
	 * @param function
	...
	/**
	 * Check that all labels defined in the specified top-level function are also
	 * used. This method requires that all relevant AST nodes have been associated
	 * with their symbol table {@link Constants#SCOPE scopes} and that all used
	 * labels have been annotated as {@link Constants#ATT_USED used}.
	 *
	 * @param function
	 *          The function to check.
	 */
	...
	public void visitCaseLabel(GNode n) {
		...
		} else if ((!t1.hasConstant()) || ((null != t2) && (!t2.hasConstant()))) {
	...
	public void visitLocalLabelDeclaration(GNode n) {
		for (Object o : n) {
			...
			if (table.current().isDefinedLocally(name)) {
				...
				define(table.current(), name,
						new LabelT(id).locate(n).attribute(Constants.ATT_UNINITIALIZED));
			}
		}
	}
	...
	public Type visitReturnStatement(GNode n) {
		...
		if (result.hasTag(Tag.VOID)) {
			...
			if (null != t1) {
				...
				runtime.warning(
						"'return' with no value, in function returning " + "non-void", n);
			}
		}
	...
	/**
	 * Process the specified expression node. This method simply dispatches this
	 * visitor on the specified node and returns the resulting type. It is
	 * typically used from other visitors nested in this class.
	 *
	 * @param n
	 *          The expression node.
	 * @return The corresponding type.
	...
	public Type visitConditionalExpression(GNode n) {
		...
		if (null == t2)
			t2 = t1; // Allow for GCC's omitted middle operand.
		...
		} else if (r2.isPointer() && t3.hasConstant()
				&& t3.getConstant().isNull()) {
			result = cond1 ? c().qualify(r2, t2) : ErrorT.TYPE;
			result = valueConditional(result, t1, t2, t3);
	...
	/**
	 * Determine a conditional expression's value. If a conditional expression's
	 * value can be statically determined, this method annotates the expression's
	 * result with that value.
	 *
	 * @param result
	 *          The result type.
	 * @param t1
	 ...
	 * @param t2
	 *          The consequence's type.
	 * @param t3
	 *          The alternative's type.
	...
	public Type visitEqualityExpression(GNode n) {
		...
		} else if (r1.isPointer() && t2.hasConstant()
				&& t2.getConstant().isNull()) {
			result = NumberT.INT;
		...
		} else if (t1.hasConstant() && t1.getConstant().isNull()
				&& r2.isPointer()) {
			result = NumberT.INT;
		...
		if (r1.isError() || r2.isError()) {
			...
			if (t1.hasConstant() && t2.hasConstant()) {
				...
				if (c().isIntegral(result)) {
					...
					if (t1.getConstant().isReference()
						...
						if ("+".equals(op)) {
							// ref + int
							result = result.constant(t1.getConstant().refValue()
							...
							&& t2.getConstant().isReference()) {
						// int + ref
						result = result.constant(t2.getConstant().refValue()
						...
						try {
							...
							// int + int
							result = result
									.constant("+".equals(op) ? i1.add(i2) : i1.subtract(i2));
						} catch (IllegalStateException x) {
							// ref + ref
							result = result.constant(new StaticReference(result));
						}
			...
			if (r1.isPointer() && c().isIntegral(r2)) {
					...
					if (c().hasConstRef(t1) && t2.hasConstant()) {
						...
						try {
							result = result.constant(
									c().getConstRef(t1).add(t2.getConstant().bigIntValue()));
						} catch (IllegalStateException x) {
							result = result.constant(new StaticReference(result));
						}
					}
				...
				if (ensurePointerArithmetic(n, r2)) {
					...
					if (t1.hasConstant() && c().hasConstRef(t2)) {
						...
						try {
							result = result.constant(
									c().getConstRef(t2).add(t1.getConstant().bigIntValue()));
						} catch (IllegalStateException x) {
							result = result.constant(new StaticReference(result));
						}
					}
			...
			if (ensurePointerArithmetic(n, r1)) {
				...
				if (c().hasConstRef(t1) && t2.hasConstant()) {
					...
					try {
						result = result.constant(
								c().getConstRef(t1).subtract(t2.getConstant().bigIntValue()));
					} catch (IllegalStateException x) {
						result = result.constant(new StaticReference(result));
					}
				}
	...
	public Type visitCastExpression(GNode n) {
		...
		// ignoring qualifiers.
		final boolean gcc = (!pedantic && r1.hasStructOrUnion()
				&& r2.hasStructOrUnion() && c().equal(r1, r2));
		Type result;
			...
			if (!cond2) {
				...
				if (c().hasConstRef(t2)) {
					...
					if (pt1.equals(pt2)) {
						...
						result = result
								.constant(new CastReference(pt1, c().getConstRef(t2)));
					}
				}
			}
		}
	...
	protected Type processOffset(Type base, GNode selection) {
		if (selection.hasName("PrimaryIdentifier")) {
			...
			if (!base.hasConstant() || !index.hasConstant())
				return element;
	...
	public Type visitTypeCompatibilityExpression(GNode n) {
		...
		final Type result = NumberT.INT.annotate()
				.constant(c().compose(t1, t2, pedantic).isError() ? BigInteger.ZERO
						: BigInteger.ONE);
		...
		if (ensureArithmetic(n1, t1)) {
			...
			if (t1.hasConstant()) {
				...
				try {
					if (c().isIntegral(result)) {
						result = result.constant(
								c().mask(t1.getConstant().bigIntValue().negate(), result));
					} else {
	...
	public Type visitAddressExpression(GNode n) {
		...
		if (n1.hasName("IndirectionExpression")) {
			...
			if (type.isError()) {
				...
				if (base.resolve().isArray() && base.hasShape()
						&& base.getShape().isConstant()) {
					// Account for pointer decay.
					ref = base.getShape();
				...
				if (null != ref && index.hasConstant()) {
					...
					try {
						result = result
								.constant(ref.add(index.getConstant().bigIntValue()));
					} catch (IllegalStateException x) {
						result = result.constant(new StaticReference(result));
					}
				}
	...
	/**
	 * Process the specified address expression. This method assumes that the
	 * specified node neither is an indirection nor a subscript expression.
	 *
	 * @param n
	 *          The node.
	 * @param type
	...
	protected Type processAddress(GNode n, Type type) {
		...
		if (resolved.isError()) {
			...
			if (type.hasVariable() && type.toVariable().hasWidth()) {
				...
				runtime.error(
						"address of register " + toDescription(n.getNode(0)) + " requested",
						n);
				return ErrorT.TYPE;
	 ...
	 *          The type.
	 * @param warn
	 *          The flag for whether to emit any warnings.
	 * @return The resulting type.
	 */
	...
	public Type visitPreincrementExpression(GNode n) {
		...
		Type result;
		if (ensureScalar(n1, t1) && ensurePointerArithmetic(n, t1)
				&& ensureLValue(n1, t1)) {
			result = t1.resolve();
		} else {
			result = ErrorT.TYPE;
	 ...
	 * @param index
	 *          The index type.
	 * @return The corresponding element type or an error type in case of an
	 *         error.
	 */
	...
	/**
	 * Process a component selection. This method extracts the member type for the
	 * member with the specified name from the specified struct or union type.
	 *
	 * @param node
	 *          The node.
	 * @param type
	 ...
	 * @param indirect
	 *          The flag for whether the component selection is indirect.
	 * @return The corresponding member type or an error type in case of a
		...
		if (base.hasError()) {
			...
			if (result.isError()) {
				runtime.error("'" + (tag.isStruct() ? "struct " : "union ")
						+ tag.getName() + "' has no member named '" + name + "'", node);
				return ErrorT.TYPE;
			...
			if (null == t1) {
				...
				if (pedantic) {
					...
					t1 = new FunctionT(NumberT.INT, new ArrayList<Type>(0), false)
							...
							.attribute(Constants.ATT_IMPLICIT).annotate()
							.attribute(Constants.ATT_STORAGE_EXTERN);
					...
					final Type extern = lookupExtern(name);
					if (null == extern) {
						// runtime.warning("implicit declaration of function '"+name+"'",
						// n);
	...
	public Type visitArrayDeclarator(GNode n) {
		return (Type) dispatch(n.getNode(0));
	}

	public Type visitAttributedDeclarator(GNode n) {
		return (Type) dispatch(n.getNode(1));
	}

	public Type visitFunctionDeclarator(GNode n) {
		return (Type) dispatch(n.getNode(0));
	}

	public Type visitPointerDeclarator(GNode n) {
		return (Type) dispatch(n.getNode(1));
	}
	...
	/**
	 * Check that the specified type is well-formed. This method checks that the
	 * specified type is not an array of functions, not a function returning an
	 * array or a function, and not a pointer to either of the first two types.
	 *
	 * @param node
	 *          The declaring node.
	 * @param name
		...
		if (type.isArray()) {
			...
			if (element.isFunction()) {
				if (null == name) {
					...
					runtime.error("'" + name + "' declared as array of functions", node);
				}
				return false;
		...
		if (type.isFunction()) {
			...
			if (result.isArray()) {
				if (null == name) {
					...
					runtime.error(
							"'" + name + "' declared as function returning " + "an array",
							node);
				}
				return false;
				...
				if (null == name) {
					...
					runtime.error(
							"'" + name + "' declared as function returning " + "a function",
							node);
				}
				return false;
	...
	/**
	 * Compose the specified type with the type of the previous declaration. This
	 * method composes compatible object and function types with each other, while
	 * also reporting any errors.
	 *
	 * @param decl
	 *          The current declaration's node.
	 * @param name
	 *          The identifier name.
	 * @param type
	 *          The current declaration's type.
	 * @param previous
	 *          The previous declaration's type.
	 * @param isFuncDef
	 *          The flag for whether the composition is for a function definition.
	 * @return The composite type or {@link ErrorT#TYPE} if the two types are not
	 *         compatible.
	 */
	public Type compose(GNode decl, String name, Type type, Type previous,
		...
		if (previous.hasAttribute(Constants.ATT_STORAGE_EXTERN)
				|| (!type.hasAttribute(Constants.ATT_STORAGE_EXTERN)) || isFuncDef) {
	 ...
	 * @param specifiers
	 *          The specifiers.
	 * @return The corresponding declaration specifier or <code>null</code> if no
	 *         such specifier exists.
	 */
	public static GNode getSpecifier(String name, GNode specifiers) {
		for (Object o : specifiers) {
			final GNode specifier = GNode.cast(o);
			if (specifier.hasName(name))
				return specifier;
		}
		return null;
	}
	...
	/**
	 * Create a new sequence of specifiers. This factory method should be used
	 * instead of allocating an instance of {@link Specifiers} directly, so that
	 * the processing of declaration specifiers can be customized by subclasses.
	 *
	 * @param specifiers
	 *          The node holding the declaration specifiers (which may be
	 *          <code>null</code>).
	...
	/**
	 * Convert the specified GCC attribute specifier or GCC attribute specifier
	 * list to an xtc attribute list. If the specified node is <code>null</code>,
	 * this method returns an empty list. Otherwise, it returns a newly allocated,
	 * unsealed list.
	 *
	 * @param spec
	 *          The GCC attribute specifier (list).
	...
	private List<Attribute> toAttributeList(GNode spec, List<Attribute> result) {
		if (spec.hasName("AttributeSpecifier")) {
			if (null != spec.get(0)) {
				for (Object o : spec.getGeneric(0)) {
					...
					if (!seenError) {
						result.add(
								new Attribute(Constants.NAME_GCC, new Attribute(name, value)));
					}
				}
			}
			return result;
			...
			for (Object o : spec)
				toAttributeList(GNode.cast(o), result);
			return result;
	...
	/**
	 * Convert the specified node to an attribute value. This method converts a
	 * node representing a GCC attribute's value to the actual attribute's value:
	 * <ul>
	 *
	 * <li>If the node is <code>null</code> or an empty expression list, this
	 * method returns <code>null</code>.</li>
	 *
	 * <li>If the node is an expression list with one element, it returns the
	 * result of recursively processing that element.</li>
	 *
	 * <li>If the node is an expression list with more than one element, it
	 * recursively processes each element, returning a new Java collections
	 * framework list.</li>
	 *
	 * <li>If the node is a primary identifier, it returns the corresponding
	 * symbol (without checking that the symbol is defined).</li>
	 *
	 * <li>If the node is a constant expression, it returns the corresponding
	 * value.</li>
	 *
	...
	public Object toAttributeValue(GNode node) {
		...
		if (node.hasName("ExpressionList")) {
			if (0 == node.size()) {
				...
				for (Object o : node)
					l.add(toAttributeValue(GNode.cast(o)));
				return l;
			}
	...
	/**
	 * Get the parameter types for the specified parameter declaration. As a side
	 * effect, this method reports any errors in a parameter's declaration
	 * specifiers, any void parameters (besides a single void specifier), any
	 * multiple parameters in old-style declarations, and any parameter
	 * redefinitions in new-style declarations.
	 *
	 * @param parameters
	 *          The parameters, which may be <code>null</code>.
	 * @return The corresponding function type with the argument types and
	 *         variable flag filled in.
	 */
		...
		if (null == parameters) {
			...
			if (isVoidParameterTypeList(parameters)) {
				...
				for (Object o : parameters) {
					...
					final Specifiers spec = newSpecifiers(param.getGeneric(0), false);
					Type type = getDeclaredType(true, spec.getBaseType(), decl);
			...
			throw new AssertionError(
					"Unrecognized parameter representation: " + parameters);
		}
	}
	 ...
	 *          The base type.
	 * @param declarator
	 *          The declarator, which may be abstract or <code>null</code>.
	...
	/**
	 * Get the declared type. In addition to determining the declared type, this
	 * method checks that any static storage class specifier or type qualifiers
	 * only appear in an array declarator if the declarator is part of a parameter
	 * declaration.
	 *
	 * @param isParam
	 *          The flag for whether the type is a parameter type.
	 * @param base
	 *          The base type.
	 * @param declarator
	 *          The declarator, which may be abstract or <code>null</code>.
	...
	public Type getDeclaredType(final boolean isParam, final Type base,
		...
		return (null == declarator) ? base : (Type) new Visitor() {
					...
					if (size.hasError()) {
						...
						try {
							...
							if (null == id) {
								...
								runtime.warning(
										"can't compute size of array '" + id.getString(0) + "'",
										GNode.cast(expr));
							}
						...
						if (value.compareTo(BigInteger.ZERO) == 0) {
							if (pedantic) {
								...
								if (null == id) {
									...
									runtime.error(
											"ISO C forbids zero-size array '" + id.getString(0) + "'",
											GNode.cast(expr));
								}
							}
							...
							if (null == id) {
								...
								runtime.error(
										"size of array '" + id.getString(0) + "' is negative",
										GNode.cast(expr));
							}
							return new ArrayT(element, 0);
							...
							if (null == id) {
								...
								runtime.error(
										"size of array '" + id.getString(0) + "' is too large",
										GNode.cast(expr));
							}
							return new ArrayT(element, 0);
			...
			public Object visitAttributedDeclarator(GNode n) {
				if (null != n.get(0))
					list1 = toAttributeList(n.getGeneric(0));
				if (null != n.get(2))
					list2 = toAttributeList(n.getGeneric(2));
	 ...
	 * @param declarator
	 *          The declarator, which may be abstract or <code>null</code>.
	 * @return The simple declarator node representing the declared identifier or
	 *         <code>null</code> if the specified declarator is abstract or
	 *         <code>null</code>.
	 */
	...
	/**
	 * Get the innermost function declarator from the specified declarator. The
	 * innermost function declarator's parameters are the parameters for a
	 * function declaration or definition.
	 *
	 * @param declarator
	 *          The declarator.
	 * @return The innermost function declarator or <code>null</code> if the
	 *         declarator does not contains a function declarator.
	 */
	 ...
	 *
	 * @param node
	 * @return The innermost identifier or <code>null</code> if the node does not
	...
	/**
	 * Determine whether the specified parameter type list represents a function
	 * taking no arguments. This method checks whether the parameter type list
	 * contains a single parameter without a declarator, whose specifiers either
	 * contains a void type specifier or a typedef name aliasing void. Though the
	 * parameter type list may still be malformed, e.g., contain a qualifier,
	 * storage class, or another type specifier.
	 *
	 * @param parameters
	 *          The parameter type list.
	 * @return <code>true</code> if the corresponding function takes no arguments.
	 */
	public boolean isVoidParameterTypeList(GNode parameters) {
		assert parameters.hasName("ParameterTypeList");

		if (null != parameters.get(1))
			return false;
		parameters = parameters.getGeneric(0);
		if (1 != parameters.size())
			return false;
		final GNode parameter = parameters.getGeneric(0);
		if (null != parameter.get(1))
			return false;
	...
	/**
	 * Determine whether the specified left-hand type can be initialized from the
	 * specified right-hand type.
	 *
	 * @param t1
	 *          The left-hand type.
	 * @param t2
	...
	/**
	 * Process the assignment. This method determines the resulting type when
	 * assigning a value of the specified right-hand type to an object with the
	 * specified left-hand type. It does not check that the left-hand side
	 * represents a modifiable lvalue.
	 *
	 * @param init
	 *          The flag for whether the assignment is in an initializer.
	 * @param op
			...
			if (r2.isPointer()) {
				...
				if (c().equal(ptr1, ptr2) || ptr1.isVoid() || ptr2.isVoid()) {
					if (c().hasQualifiers(pt1, pt2) || c().isStringLiteral(t2)) {
						...
						runtime.error(
								op + " discards qualifiers from pointer target " + "type", n);
						result = ErrorT.TYPE;
					...
					if (pedantic) {
						...
						// GCC extension.
						runtime.warning(
								"pointer targets in " + op + " differ in signedness", n);
						result = r1;
					}
	...
	/**
	 * Ensure that the specified node with the specified type represents a
	 * modifiable lvalue.
	 *
	 * @param n
	 *          The node.
	 * @param t
	 *          The type.
	 * @return <code>true</code> if the specified node represents a modifiable
	 *         lvalue.
	 */
	 ...
	 * @param t
	 *          the type.
	 * @return <code>true</code> if the specified node represents a scalar.
	 */
	...
	/**
	 * Ensure that the specified node represents valid pointer arithmetic for the
	 * specified type. If the specified type is a pointer, this method ensures
	 * that the pointer points to a complete type. For all other types besides the
	 * error type, it returns <code>true</code>.
	 *
	 * @param n
	 *          The node.
	 * @param t
	 *          The type.
	 * @return <code>true</code> if the specified node represents valid pointer
	 *         arithmetic.
	 */
	...
	/**
	 * Ensure that the specified node with the specified type represents an
	 * arithmetic type.
	 *
	 * @param n
	 *          The node.
	 * @param t
	 *          The type.
	 * @return <code>true</code> if the specified node represents an arithmetic
	 *         type.
	 */
	public boolean ensureArithmetic(Node n, Type t) {
		if (t.hasError()) {
			...
			runtime.error(
					"invalid " + toDescription(n) + " where arithmetic value required",
					n);
			return false;
		} else {
			return true;
	...
	/**
	 * Ensure that the specified note with the specified type represents an
	 * integer.
	 *
	 * @param n
	 *          The node.
	 * @param t
	 *          The type.
	 * @return <code>true</code> if the specified node represents an integer.
	 */
	 ...
	 * @param n
	 *          The node.
	 * @return The corresponding name or <code>null</code> if the name cannot be
	 *         determined.
	 */
	...
	/**
	 * Determine whether the specified scope name represents a function or macro
	 * scope.
	 *
	 * @param name
	 *          The name.
	 * @return <code>true</code> if the specified name represents a function or
	 *         macro scope.
	 */
	 ...
	 * @param name
	 *          The name.
	 * @return The corresponding type or <code>null</code> if no such binding
	 *         exists.
	 */
	...
	/**
	 * Report a previous declaration or definition. If the specified type has a
	 * location, this method prints that location as a previous declaration or
	 * definition.
	 *
	 * @param name
	 *          The name.
	 ...
	 * If the specified type has a location, this method prints that location as a
