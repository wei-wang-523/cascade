...
public interface IRBasicBlock {
	...
	public static enum Type {
		...
		LOOP,
		/**
		 * A loop init. The block has init statements or not (while statement) and
		 * only one successor: the loop
		 */
		LOOPINIT,
		/**
		 * A loop exit. The block has no statements and only one predecessor: the
		 * loop
		 */
		LOOPEXIT,
		/**
		 * A label block. The block has no statements and only one successor, and
		 * may many predecessors.
		 */
		LABEL,
		/**
		...
		SWITCH,
		/**
		 * A swich-merge block. The block merges the N paths of following a switch
		 * block.
		 */
		MERGE,
		/**
	...
	/**
	 * Add a "virtual" location (i.e., one not associated with a statement) to
	 * this block. This is useful for for making sure if and loop tests have an
	 * associated location.
	 * 
	 * @param loc
	 *          the location to add to this block
	 */
	 ...
	 * be created, with the statements from this block that come after the
	 * position. Returns the new block.
	 */
