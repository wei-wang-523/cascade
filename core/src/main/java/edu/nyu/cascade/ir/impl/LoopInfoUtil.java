...
public class LoopInfoUtil {
	...
	public static Loop getLoop(
			IRControlFlowGraph cfg, final IRBasicBlock loopHeader) {
		...
		Iterable<? extends IREdge<? extends IRBasicBlock>> backEdges = Iterables.filter(
				cfg.getIncomingEdges(loopHeader), new Predicate<IREdge<?>>() {
					@Override
  ...
  private static Collection<IREdge<?>> detectBackEdge(
  		Deque<IRBasicBlock> stack, 
  		Set<IRBasicBlock> visited, 
  		IRControlFlowGraph cfg,
  		IRBasicBlock currBlock) {
  	Collection<IREdge<?>> backEdges = Sets.newHashSet();
  	if(visited.contains(currBlock)) return backEdges;
  	...
  	for(IREdge<?> out : cfg.getOutgoingEdges(currBlock)) {
			...
			if(stack.contains(dest)) {
				...
				Collection<IREdge<?>> resEdges = detectBackEdge(stack, visited, cfg, dest);
