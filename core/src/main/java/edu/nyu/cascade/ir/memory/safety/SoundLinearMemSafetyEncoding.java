...
public class SoundLinearMemSafetyEncoding extends AbstractMemSafetyEncoding {
	...
	private SoundLinearMemSafetyEncoding(ExpressionEncoding encoding,
	...
	public static SoundLinearMemSafetyEncoding create(ExpressionEncoding encoding,
			IRDataFormatter formatter) {
	...
	// state.getPredicateMap();
	// Collection<Entry<Expression, Collection<Expression>>> entries =
	// Sets.newHashSet(predMap.entries());
	...
	// })) continue;
	//
	// List<Expression> argsPrime =
	// Lists.newArrayListWithExpectedSize(args.size());
	...
	// }
	//
	// replaceLabelsInSafetyPredicate(Kind.VALID_ACCESS, state, labels,
	// substLabels);
	// replaceLabelsInSafetyPredicate(Kind.VALID_ACCESS_RANGE, state, labels,
	// substLabels);
	// replaceLabelsInSafetyPredicate(Kind.PRE_DISJOINT, state, labels,
	// substLabels);
	...
	protected final void propagatePreDisjoint(
		...
		BooleanExpression toPreDJPrime = fromPreDJ
				.and(toPreDJ.subst(candidate, substitute));
	...
	protected final Collection<BooleanExpression> initAssumptions() {
		...
		{ // DISJOINT
			FunctionExpression func = exprManager.functionDeclarator(
					Kind.DISJOINT.name(), exprManager.functionType(addrType, sizeType,
			...
			if (Preferences.PROVER_Z3.equals(tpProviderName)) {
				...
				Expression boundVar1 = exprManager.boundVar(sizeVarName, sizeType,
						true);
				Expression lamExpr = exprManager
						.lambda(Lists.newArrayList(boundVar0, boundVar1), encoding.tt());
		...
		{ // VALID_ACCESS_RANGE
			FunctionExpression func = exprManager.functionDeclarator(
					Kind.VALID_ACCESS_RANGE.name(), exprManager.functionType(addrType,
							sizeType, exprManager.booleanType()),
					false);
			...
			if (Preferences.PROVER_Z3.equals(tpProviderName)) {
				...
				Expression boundVar1 = exprManager.boundVar(sizeVarName, sizeType,
						true);
				Expression lamExpr = exprManager
						.lambda(Lists.newArrayList(boundVar0, boundVar1), encoding.ff());
				assumps.add(func.eq(lamExpr));
			}
		}
