...
public class HoareStateFactory<T> implements StateFactory<T> {	
  ...
  public StateExpression resolvePhiNode(Collection<StateExpression> preStates) {
	  ...
	  Map<Expression, Multimap<Expression, BooleanExpression>> hoareGuardTable = Maps.newHashMap();
	  ...
	  for(Expression key : keySet) {
	  	Multimap<Expression, BooleanExpression> guardHoareMap = hoareGuardTable.get(key);
  ...
  public <X> void setLabelAnalyzer(PreProcessor<X> preprocessor) {
	...
	public void propagateState(StateExpression cleanState, StateExpression stateArg) {
  	...
  	stateFactory.getSubstPredicatesPair(cleanState, stateArg, fromPredicates, toPredicates);
		stateFactory.substPredicatesConstraint(cleanState, fromPredicates, toPredicates);
  ...
  public final Collection<BooleanExpression> getAssumptions() {
	...
	private void propagateMap(StateExpression fromState, StateExpression toState) {		
		...
		for(Expression idx : fromHoareMap.keySet()) {
			if(toHoareMap.containsKey(idx)) continue;
