...
public abstract class AbstractStateExpression implements StateExpression {
  private final Multimap<String, BooleanExpression> asserts = HashMultimap.create();
  ...
  /**
   * Keep record of local variables and newly allocated regions with 
   * option -sbe(small block based encoding) enabled
   */
  ...
  private final Map<Expression, Expression> hoareValues = Maps.newHashMap();
	...
	@Override
	final public void setAssertions(Multimap<String, BooleanExpression> assertions) {
		asserts.putAll(assertions);
	}
	
	@Override
	final public void addVar(VariableExpression variable) {
		if(!Preferences.isSet(Preferences.OPTION_SBE)) return;
		vars.add(variable);
	}
