...
public class TraceNode implements IRTraceNode {	
  ...
  public String toString() {
  	StringBuilder sb = new StringBuilder().append(id).append(" ").append(labels);
    return sb.toString();
  }
	...
	private void evalueateTraceExpr(final ExpressionManager exprManager) {
		if(stmtTraceExprMap.isEmpty()) return;
		
		final TheoremProver theoremProver = exprManager.getTheoremProver();
		Collection<IRStatement> statements = Collections.unmodifiableSet(stmtTraceExprMap.keySet());
