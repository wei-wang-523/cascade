...
public abstract class AbstractStmtMemSafetyEncoding implements IRMemSafetyEncoding {
	...
	protected AbstractStmtMemSafetyEncoding(ExpressionEncoding encoding, IRDataFormatter formatter) {
  ...
  public void propagateSafetyPredicates(SingleLambdaStateExpression fromState, 
	...
	final void initSafetyPredicate(Kind kind, SingleLambdaStateExpression state, Expression ptrVar, Expression sizeVar) {
		...
		switch(kind) {
		case DISJOINT: {
		  PredicateClosure closure = suspend(null, 
		  		encoding.tt(), ptrVar, sizeVar);
