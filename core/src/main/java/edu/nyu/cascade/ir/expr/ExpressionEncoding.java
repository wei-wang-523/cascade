 ...
 * 
 * Operations on arrays are optional: arrays are treated here as first-class
 * values, which may not be appropriate in some encodings. (E.g., in a
 * bit-precise encoding for C programs, the representation of arrays will be
 * embedded in the memory model, not the expression encoding.)
 * 
...
public interface ExpressionEncoding {
	 ...
	 * 
	 * @param e
	 *          a boolean- or pointer- integer-encoded expression
	 ...
	 *          an expression
	 * @param rhs
	 *          an expression (of the same encoded type as <code>lhs</code>)
	 * @return a boolean-encoded expression
	 */
	 ...
	 * 
	 * @param array
	 *          an array in the encoding
	 * @param index
	 *          an expression (of the same encoded type as <code>array</code>'s
	 *          indices)
	 * @return an expression of the same encoded type as <code>array</code>'s
	 *         elements
	 */
	 ...
	 *          an expression
	 * @param rhs
	 *          an expression of the same encoded types as <code>lhs</code>
	 * @return a boolean-encoded expression
	 */
	 ...
	 * 
	 * @param e
	 *          an expression of tuple type (i.e., e.isTuple() should return true)
	 * @return an pointer-encoded expression
	 */
	...
	/**
	 * <code>base <= base + size</code> while <code>+</code> is modular arithmetic
	 * in fixed-size bitvector encoding
	 * 
	 * @param base
	 * @param size
	 *          must be positive
	...
	/**
	 * Regions <code>[base1, base1 + size1)</code> and
	 * <code>[base2, base2 + size2)</code> are disjoint.
	 * 
	 * @param base1
	 * @param size1
	 * @param base2
	...
	/**
	 * A bound variable in the encoding. Creates the bound variable if it doesn't
	 * already exist. If a bound variable with the same name already exists, it
	 * will be returned if <code>fresh</code> is false; a new bound variable with
	 * a mangled name will be created if <code>fresh</code> is true.
	 * 
	 * @param name
	 ...
	 * @param fresh
	 *          if true, the bound variable is guaranteed to be unique in the
	 *          encoding
	...
	/**
	 * A bound expression in the encoding. Creates the bound expression if it
	 * doesn't already exist. If a bound expression with the same name already
	 * exists, it will be returned if <code>fresh</code> is false; a new bound
	 * expression with a mangled name will be created if <code>fresh</code> is
	 * true.
	 * 
	 * @param name
	 ...
	 * @param fresh
	 *          if true, the bound expression is guaranteed to be unique in the
	 *          encoding
	...
	/**
	 * Convert the type of <code>lhs</code> and <code>rhs</code> with type
	 * <code>lhsType
