...
public interface StateFactory<T> {
	 ...
	 * assigned to <code>ptr</code>, <code>ptrNode</code> is the source node of
	 * <code>ptr</code>
	 */
	 ...
	 * is the returned pointer of memset. Here we actually checks if [ptr,
	 * ptr+size) are valid access with the region pointed by <code>ptr</code>.
	 * Thus in multi-lambda encoding, the partition we need to check the predicate
	 * is not the partition of <code>ptrNode</code>, but the partition pointed by
	 * it.
	 * 
	 * @return
	 */
	...
	/**
	 * Substitute the element state variable in <code>cleanState</code> with
	 * <code>stateArg</code>
	 * 
	 * @param cleanState
	 * @param stateArg
	 ...
	 * <code>state</code> with variable size <code>rval</code>. If
	 * <code>state</code> is a multiple state or multiple lambda state, and
	 * doesn't contain the element state for <code>lval</code>, a new element
	 * state will be added into <code>state
	...
	/**
	 * Create a clone of itself to keep clean of side-effect caused by following
	 * operations. Used in the <code>SimplePathEncoding</code> to analyze
	 * statements. Otherwise, the side-effect caused of the expression evaluation
	 * will change the input state, which might also be the input state of other
	 * statements (e.g. in ite-branch), the side-effect of the if-branch
	...
	/**
	 * Look up the size of variable length array <code>ptr</code> with source node
	 * <code>node</code>
	 * 
	 * @param state
	 * @param ptr
