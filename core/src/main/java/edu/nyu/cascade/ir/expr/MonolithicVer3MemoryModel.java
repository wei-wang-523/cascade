...
public class MonolithicVer3MemoryModel extends AbstractMemoryModel {
   ...
   */
  public static MonolithicVer3MemoryModel create(
      ExpressionEncoding encoding)
      throws ExpressionFactoryException {
    Preconditions.checkArgument(encoding instanceof PointerExpressionEncoding);
    return new MonolithicVer3MemoryModel(encoding);
  }
...
//    Preconditions.checkArgument(stateType.equals(memoryVar.getType()));
  ...
  private String getArrName(Expression expr) {
    ...
    } else if(getRefType().equals(expr.getType())) {      
  ...
  private boolean isPointer(Expression expr) {
    return isPointer(expr.getNode());
  }
  
  private boolean isPointer(Node node) {
