...
/**
 * Formatter for vari-cell heap encoding with linear pointer type compute the
 * size of the partitioned memory array's element type based on the size of each
 * xtc type
 * 
 * @author Wei
 *
...
public class VariCellLinearFormatter extends AbstractDataFormatter {
	...
	public static VariCellLinearFormatter create(ExpressionEncoding encoding) {
		Preconditions
				.checkArgument(Preferences.isSet(Preferences.OPTION_BYTE_BASED));
	...
	/**
	 * @param type
	 *          is not used in multi-cell bit-vector formatter
	 */
	@Override
	...
	public BooleanExpression memorySet(ArrayExpression memory, Expression region,
		...
		Expression valueInCellSize = encoding
