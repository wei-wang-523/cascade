...
public class SingleLambdaStateFactory<T> extends AbstractStateFactory<T> {
  ...
  public SingleLambdaStateExpression freshState() {
    SingleLambdaStateExpression lambdaState = SingleLambdaStateExpression.create(
    		singleStateFactory.freshSingleState());
  ...
  public SingleLambdaStateExpression copy(StateExpression state) {
  	...
  	SingleLambdaStateExpression newState = SingleLambdaStateExpression.create(singleStateCopy);
  	...
  	newState.putAllSafetyPredicateClosures(singleLambdaState.getSafetyPredicateClosures());
	...
	@Override
	protected void propagateMemSafetyPredicates(StateExpression stateArg, StateExpression cleanState) {
		memSafetyEncoding.propagateSafetyPredicates(
				stateArg.asSingleLambda(), cleanState.asSingleLambda());
	}
	...
	protected void getSubstPredicatesPair(
		...
		Multimap<Expression, Collection<Expression>> cleanPredicateMap = 
				cleanState.asSingleLambda().getPredicateMap();
		
		SingleLambdaStateExpression preLambdaState = preState.asSingleLambda();
		
		for(Entry<Expression, Collection<Expression>> entry : cleanPredicateMap.entries()) {
			...
			Expression toExpr = memSafetyEncoding.applyUpdatedPredicate(preLambdaState, predicate, args);
			
			if(!fromExpr.equals(toExpr)) {
				fromPredicates.add(fromExpr);	toPredicates.add(toExpr);
			}
		}
	}
  ...
  @Override
	protected void substSafetyPredicates(StateExpression state,
  	...
  	SingleLambdaStateExpression singleLambdaState = state.asSingleLambda();
		...
		Multimap<Expression, Collection<Expression>> predicateMap = singleLambdaState.getPredicateMap();
		for(Entry<Expression, Collection<Expression>> entry : ImmutableList.copyOf(predicateMap.entries())) {
			Collection<Expression> values = entry.getValue();
			if(values.isEmpty()) continue;
			
			predicateMap.remove(entry.getKey(), values);
			Collection<Expression> valuesPrime = Lists.newArrayListWithCapacity(values.size());
			for(Expression value : values) valuesPrime.add(value.subst(fromElems, toElems));
			predicateMap.put(entry.getKey(), valuesPrime);
		}
		
		Map<String, BooleanExpression> safetyPredicates = singleLambdaState.getSafetyPredicates();
		for(String label : ImmutableSet.copyOf(safetyPredicates.keySet())) {
			BooleanExpression safetyPredicate = safetyPredicates.get(label);
			BooleanExpression safetyPredicatePrime = safetyPredicate.subst(fromElems, toElems)
					.asBooleanExpression();
			safetyPredicates.put(label, safetyPredicatePrime);
		}
		
		Map<String, PredicateClosure> safetyPredicateClosures = singleLambdaState.getSafetyPredicateClosures();
		for(String label : ImmutableSet.copyOf(safetyPredicateClosures.keySet())) {
			PredicateClosure safetyPredicateClosure = safetyPredicateClosures.get(label);
			...
			PredicateClosure safetyPredicateClosurePrime = memSafetyEncoding.suspend(
					safetyPredicateClosure.getUninterpretedFunc(), bodyPrime, safetyPredicateClosure.getVars());
			safetyPredicateClosures.put(label, safetyPredicateClosurePrime);
		}
	}

	void addStackVar(SingleLambdaStateExpression lambdaState, Expression stVarExpr, Expression stVarSize) {
	  memSafetyEncoding.updateStackMemSafetyPredicates(lambdaState, stVarExpr, stVarSize);
	}
	...
	SingleLambdaStateExpression freshSingleLambdaState(String elemName, ArrayType[] elemTypes) {
		SingleLambdaStateExpression fresh = SingleLambdaStateExpression.create(
