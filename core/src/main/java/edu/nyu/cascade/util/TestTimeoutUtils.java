...
public class TestTimeoutUtils {
	 ...
	 *          a {@link File} task
	 */
	public static void checkDirectory(Scheduler scheduler, File dir,
			FilenameFilter filter, TaskBuilder<File> builder) {
		// Get all test files
	...
	public TestTimeoutUtils(List<Task<?>> tasks, long timeout) {
		...
		Iterable<Future<?>> futures = Iterables.transform(tasks,
				new Function<Task<?>, Future<?>>() {
					@Override
